...
class Runner {
  /**
   * @param {{ registry: object, store: object }} opts
   */
  ...
  async resume({ runId }) {
    ...
    const pipeline = { name: run.pipeline, gates: run.gates };
    run.status = 'running';
  ...
  async _execute(run, planStages, pipeline, opts = {}) {
    for (const planEntry of planStages) {
      ...
      try {
        ...
        const verdict = evaluateGate({ artifactRef, store: this.store, rules: pipeline.gates });
        if (!verdict.pass) {
