...
- **不覆盖调试器的实时追踪**（ETM/ITM/SWO）— 那是 `platform-cortex-registers` / `debug-diagnostic-framework` 的范畴
- **不替代仿真器单步调试** — CmBacktrace 用于无法连接仿真器时的故障定位
- 与 `debug-rtos` 互补：CmBacktrace 定位故障原因；debug-rtos 分析任务级运行态问题
- 与 `platform-cortex-registers` 互补：CmBacktrace 自动解码故障寄存器；platform-cortex-registers 提供寄存器手动查询

## 交接

- **addr2line 不可用或路径错误** → 检查是否安装了 arm-none-eabi-gcc 工具链，或从 CmBacktrace tools/ 目录下载独立版
- **故障类型无法解析** → 使用 `platform-cortex-registers` skill 手动解码 CFSR/HFSR 寄存器
- **FreeRTOS 集成问题** → 使用 `rtos-freertos` skill 检查任务栈配置
- **需要更深入的低级调试** → 使用 `debug-diagnostic-framework` skill 的五层诊断模型
- **HardFault 反复出现但无规律** → 使用 `quality-code-review` skill 做代码安全审查（ISR/竞态/栈溢出）
