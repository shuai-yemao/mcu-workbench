---
name: observability-rtt-monitor
...
### 不该激活
- 用户只需要**普通串口**日志查看（不使用 RTT 协议）→ 使用 `observability-serial-monitor`
...
- 上游：`tool-build-keil` / `tool-build-cmake` / `tool-flash-jlink`（编译烧录后查看实时日志）
- 替代：`observability-serial-monitor`（当没有 RTT 支持时回退到串口日志）
- 调试时：`debug-rtos`（RTT 日志发现 RTOS 异常后深入分析）
