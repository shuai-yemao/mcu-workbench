...
# 指定上传端口
python scripts/pio_flasher.py --flash --project-dir /path/to/project --env esp32dev --upload-port {SERIAL_PORT}
```
...
| `--env` | 构建环境名称 |
| `--upload-port` | 上传端口（如 {SERIAL_PORT}、/dev/ttyUSB0） |
