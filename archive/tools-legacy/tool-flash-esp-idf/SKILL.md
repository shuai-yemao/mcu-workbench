---
name: tool-flash-esp-idf
description: 当需要通过 ESP-IDF 工具链烧录固件到 ESP32 系列芯片，或启动调试会话时使用。当用户提到 ESP32 烧录、esptool、idf.py flash、ESP-IDF 烧录、ESP32 下载、flash ESP32 时使用。
version: "1.0.0"
---
...
- 当 `build/` 目录不存在或产物缺失时，返回 `artifact-missing`，推荐 `tool-build-esp-idf`。
- 当烧录过程中芯片无响应时，返回 `target-response-abnormal`。
- 当存在多个串口设备且无法确定目标时，返回 `ambiguous-context`。

## 平台说明

- 串口设备命名因平台而异：Linux `/dev/ttyUSB0`，macOS `/dev/cu.usbserial-*`，Windows `{SERIAL_PORT}`。
...
- 根据用户意图推荐下一步 skill：查看运行日志推荐 `observability-serial-monitor`，需要调试推荐 `debug-gdb-openocd`。

## 边界定义

### 不该激活
- 用户的目标芯片是 **STM32/nRF/GD32** 等非 ESP 系列 → 优先用 `tool-flash-openocd` / `tool-flash-jlink`
- 用户使用的是 Arduino IDE 而非 ESP-IDF → 使用 `tool-flash-platformio`
- 用户的 ESP32 工程基于 PlatformIO 构建 → 使用 `tool-flash-platformio`
- 用户只需要查看串口日志而非烧录 → 使用 `observability-serial-monitor`

### 不该做
- **禁止**在用户未确认串口设备的情况下自动选择端口（即使只有一个）
- **禁止**在 `build/` 目录不存在或产物缺失时静默跳过
- **禁止**擦除 Flash 时未提示用户确认（--erase-flash 前须确认）
- **禁止**在 ESP32 芯片处于下载模式时额外操作 GPIO0/EN 引脚

### 不该碰
- **不触碰** Flash 分区表（partition table）：不修改 sdkconfig 中的分区配置
- **不触碰** Efuse：不烧录、不读取、不修改 eFuse 位（安全熔丝不可逆）
- **不触碰** 非目标串口的其他串口设备
- **不触碰** NVS（Non-Volatile Storage）用户数据区

## 交接关系

- 当下一步要看运行日志时，将成功烧录结果交给 `observability-serial-monitor`。
- 当用户需要断点调试或崩溃分析时，将结果交给 `debug-gdb-openocd`。
- 当固件产物缺失时，推荐用户先使用 `tool-build-esp-idf` 编译。
