...
| `--port` | 串口设备路径（如 `/dev/ttyUSB0`、`{SERIAL_PORT}`） |
...
在 `tool-flash-esp-idf` skill 中，推荐工作流是：

1. 先用 `--detect` 确认环境和串口状态
2. 若 `idf.py` 不可用，提示用户手动安装 ESP-IDF
3. 首次使用时，列出探测到的串口设备供用户选择，确认波特率
4. 若构建产物缺失，推荐 `tool-build-esp-idf`
5. 使用用户确认的串口和波特率执行烧录
6. 整理结果摘要，更新 `Project Profile`，交给 `observability-serial-monitor` 或 `debug-gdb-openocd`
