...
- **Ozone**: 在系统 PATH 中可执行，或通过 `<OZONE_PATH>` 显式指定。
...
- **不处理**：固件编译、烧录（由 tool-build-keil/tool-build-cmake/tool-flash-keil 处理）
- **不处理**：J-Link 驱动安装/固件升级（由 tool-flash-jlink 的 JLink Commander 处理）
- **不处理**：RTT 应用层移植（由 observability-rtt-porting 处理）
...
| 相邻 skill | 关系 |
|-----------|------|
| `observability-rtt-porting` | Ozone 集成 RTT 输出，该 skill 负责 RTT 应用层移植 |
| `tool-flash-jlink` | Ozone 依赖 J-Link，该 skill 负责 J-Link 驱动验证 |
| `debug-diagnostic-framework` | Ozone 是五层诊断模型中的调试工具，该 skill 提供诊断方法论 |
| `platform-cortex-registers` | Ozone 实时查看内核寄存器，该 skill 提供寄存器速查参考 |
| `debug-crash-backtrace` | HardFault 定位可先用 CmBacktrace 自动分析，再用 Ozone 指令追溯确认 |
