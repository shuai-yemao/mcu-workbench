...
- 上游：`bus-uart`（AT 命令模块底层通信）
- 互补：`protocol-mqtt`（WiFi + MQTT 云连接典型方案）
- 互补：`protocol-ble`（BLE+WiFi 共存/混合配网）
- 参考：`platform-mcu-architecture`（WiFi SoC 选型）
