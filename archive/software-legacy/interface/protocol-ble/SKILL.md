...
- 用户只需要串口透传（HC-05/HC-06 模块）→ 使用 `bus-uart` + AT 指令即可
...
- 上游：`bus-uart`（BLE 透传模块底层通信）
- 互补：`system-low-power`（BLE 设备功耗优化）
- 下位：`protocol-wifi`（BLE+WiFi 共存方案）
- 参考：`platform-mcu-architecture`（BLE 芯片选型）
