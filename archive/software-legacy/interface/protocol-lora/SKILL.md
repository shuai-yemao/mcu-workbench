...
### 不该激活
- 用户需要的是短距离高速无线（WiFi/BLE）→ 使用 `protocol-wifi` / `protocol-ble`
- 用户需要的是蜂窝通信（4G/NB-IoT）→ 使用 `protocol-cellular`
...
- 上游：`bus-spi`（SX1278/SX1262 SPI 通信底层）
- 互补：`protocol-cellular`（LoRa 替代或补充方案对比）
- 互补：`system-low-power`（LoRa 节点低功耗设计）
- 参考：`platform-mcu-architecture`（LoRa 芯片选型）
