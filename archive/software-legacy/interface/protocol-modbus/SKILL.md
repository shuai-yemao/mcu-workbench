...
### 不该激活
- 用户需要的是纯串口日志查看（无 Modbus 协议）→ `observability-serial-monitor`
- 用户需要调试的是 Modbus ASCII 模式（本 skill 仅支持 RTU/TCP）
- 用户需要在 MCU 固件侧实现 Modbus 从站/主站 → `bsp-device-adaptation`
...
- 从 `tool-build-keil` / `tool-build-platformio` 烧录固件后，用此 skill 验证 Modbus 通信。
- 与 `observability-serial-monitor` 互补：observability-serial-monitor 查看串口原始输出，protocol-modbus 进行协议级调试。
