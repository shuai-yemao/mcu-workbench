---
name: platform-stm32-spl
description: "STM32 标准外设库开发"
---

# STM32 标准外设库 (SPL) 开发指南

> 与 `platform-stm32-hal`（HAL 框架）、`platform-peripheral-registers`（寄存器级）和 `workflow-code-porting`（SPL↔HAL 移植）互补——本 skill 专注 SPL **库架构、外设配置模式、固件维护和国产兼容 MCU 适配**。
...
> 涉及代码规范审查 → 串联 `coding-standards` + `quality-code-review`
...
```
platform-stm32-spl (本 skill — SPL 参考手册)
    ↓ 分场景：
    ├─ 想改用 HAL → platform-stm32-hal (HAL 框架)
    ├─ 想操作寄存器 → platform-peripheral-registers (寄存器级)
    ├─ 需要代码审查 → quality-code-review (ISR/DMA/并发审查)
    ├─ 需要迁移计划 → workflow-code-porting (SPL↔HAL 移植) [动态注册]
    └─ 需要编译烧录 → workflow (tool-build-keil → tool-flash-keil)
```

## 交接关系

- 迁移目标：`platform-stm32-hal`（SPL->HAL 升级路径）
- 辅助：`workflow-code-porting`（SPL->HAL 移植工具）
- 互补：`platform-peripheral-registers`（寄存器级操作参考）
- 审查：`quality-code-review`（ISR/DMA/并发安全审查）
- 平台：`platform-internal-flash`（Flash 分区/等待周期配置）
