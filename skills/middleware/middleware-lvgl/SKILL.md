...
- 下游：`peripheral-timer`（LVGL 定时器心跳 lv_tick_inc）
- 下游：`bus-spi`（SPI LCD 驱动底层通信）
- 下游：`peripheral-dma`（SPI DMA 传输像素数据）
- 下游：`bsp-device-adaptation`（触摸芯片驱动 FT5x06/GT911）
- 参考：`platform-mcu-architecture`（MCU 选型：是否有 DMA2D/LTDC）
