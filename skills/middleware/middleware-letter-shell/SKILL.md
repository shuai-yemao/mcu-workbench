---
name: middleware-letter-shell
description: letter_shell 嵌入式串口命令行 shell，支持命令段导出、参数解析、历史、Tab 补全与变量导出；当用户提到 letter_shell、嵌入式 shell、命令行、SHELL_EXPORT_CMD、串口命令、shellPrint、串口调试命令时使用。
---
...
UART 通道与物理收发属于 [`core-mcu`](../../core/core-mcu/SKILL.md) 或 [`mcu-platform`](../../mcu/mcu-platform/SKILL.md)；日志输出建议用 RTT/ELOG（[`tools-observability`](../../tools/tools-observability/SKILL.md)），不要在 ISR 里直接调 `shellPrint`。
