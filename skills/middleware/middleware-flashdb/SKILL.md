---
name: middleware-flashdb
description: FlashDB 嵌入式 KV/TS 数据库，基于 FAL 分区提供掉电安全的键值与时序存储；当用户提到 FlashDB、KVDB、TSDB、fdb_kv_set、fdb_kv_get、blob 读写、掉电持久化、键值存储、时序存储、Flash 数据库时使用。
---
...
存储介质规划、分区划分与相对偏移属于 FAL；KV/TS 之上的文件系统属于 [`middleware-storage`](../middleware-storage/SKILL.md)；器件协议属于 [`bsp-hal-driver`](../../bsp/bsp-hal-driver/SKILL.md)。
...
分区来源交给 [`middleware-fal`](../middleware-fal/SKILL.md)；与 FatFs 等并存时需协调同一根 SPI 总线的并发（FlashDB 数据库锁 + 下层 SPI 总线锁两层缺一不可），文件系统细节交给 [`middleware-storage`](../middleware-storage/SKILL.md)。
