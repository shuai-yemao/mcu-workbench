---
name: protocol-ymodem
description: "Ymodem 文件传输"
---
...
- **不覆盖** Flash 写入操作 → 使用 `platform-internal-flash`
- **不覆盖** CRC-16 算法实现 → 使用 `security-crc`
- 协议假设串口 8N1 模式
- 不覆盖 Xmodem/Zmodem 协议

## 交接关系

- 上游：`bus-uart`（UART 配置：波特率/中断/DMA）
- 上游：`platform-internal-flash`（接收固件后的 Flash 写入）
- 上游：`security-crc`（Ymodem 帧 CRC-16 校验算法）
- 集成：`system-bootloader`（Ymodem + Bootloader 完整 OTA 方案）
- 集成：`release-ota-package`（Ymodem + EEPROM 状态机 OTA 实现参考）
