---
name: tool-build-cmake
description: 当需要配置或构建基于 CMake 的嵌入式固件工程，调用自带脚本执行构建并定位固件产物时使用。当用户提到 CMake 编译、cmake build、交叉编译、ARM GCC 编译、嵌入式 CMake、cmake 构建时使用。
version: "1.0.0"
---
...
- 成功后推荐 `tool-flash-openocd` 或 `debug-gdb-openocd`。

## 边界定义

### 不该激活
- 工程中存在 `.uvprojx`/`.uvproj`（Keil）→ 优先用 `tool-build-keil`
- 工程中存在 `.ewp`/`.eww`（IAR）→ 优先用 `tool-build-iar`
- 工程中存在 `platformio.ini`（PlatformIO）→ 优先用 `tool-build-platformio`
- 工程是 ESP-IDF 结构（含 `CMakeLists.txt` + `main/` + `sdkconfig`）→ 优先用 `tool-build-esp-idf`
- 用户只需要生成编译数据库（compile_commands.json）而不需要实际编译 → 纯 cmake 命令足够

### 不该做
- **禁止**在无 CMakeLists.txt 时生成虚拟工程配置
- **禁止**修改用户现有的 CMakeLists.txt 或 CMakePresets.json
- **禁止**在多个预设（preset）候选时静默选择
- **禁止**使用不兼容的生成器（如在 Linux 上强制选择 Visual Studio）

### 不该碰
- **不触碰**构建产物目录外的任何源文件
- **不触碰**工具链安装目录（ARM GCC、Ninja 等系统组件）
- **不触碰** CMakePresets.json 和 CMakeLists.txt：只读取，不修改

## 交接关系

- 当下一步意图是给硬件烧录程序时，将成功构建结果交给 `tool-flash-openocd`。
