---
name: tool-build-platformio
description: 当需要通过 PlatformIO 命令行编译嵌入式工程，调用自带脚本解析环境配置、执行构建并定位固件产物时使用。当用户提到 PlatformIO 编译、pio build、PIO 编译、PlatformIO 构建时使用。
version: "1.0.0"
---
...
### 不该激活
- 工程中存在 `CMakeLists.txt` 但无 `platformio.ini`（CMake 工程）→ 优先用 `tool-build-cmake`
- 工程中存在 `.uvprojx`/`.uvproj`（Keil）→ 优先用 `tool-build-keil`
- 工程中存在 `.ewp`/`.eww`（IAR）→ 优先用 `tool-build-iar`
- 工程是 ESP-IDF 结构而非 Arduino-ESP32 框架 → 优先用 `tool-build-esp-idf`
- 用户只需要上传（烧录）而不需要编译 → 使用 `tool-flash-platformio`

### 不该做
- **禁止**修改 platformio.ini
- **禁止**在多个环境（env）候选时静默选择
- **禁止**自动安装 PlatformIO 或修改 PlatformIO 全局配置

### 不该碰
- **不触碰** platformio.ini：只读取，不修改
- **不触碰** PlatformIO 全局配置（~/.platformio/ 或 %USERPROFILE%/.platformio/）
- **不触碰** .pio/ 构建缓存中非产物文件
