---
name: tool-build-esp-idf
description: 当需要配置或构建基于 ESP-IDF 的固件工程，调用自带脚本执行 idf.py build 并定位固件产物时使用。当用户提到 ESP-IDF 编译、idf.py build、ESP32 编译、esp-idf 构建、ESP32-S3 编译时使用。
version: "1.0.0"
---
...
- 成功后推荐 `tool-flash-esp-idf`。

## 边界定义

### 不该激活
- 工程不是 ESP-IDF 结构（无 CMakeLists.txt + main/ + sdkconfig 组合特征）
- 用户使用的是 Arduino-ESP32 框架 → 优先用 `tool-build-platformio`
- 用户的目标芯片不是 Espressif 系列（非 ESP32/S2/S3/C3/C6/H2 等）
- 用户的 ESP-IDF 环境未安装且无法激活

### 不该做
- **禁止**在用户未确认目标芯片的情况下执行 `set-target`（可能覆盖已有 sdkconfig）
- **禁止**自动执行 `idf.py menuconfig`（交互式命令，必须用户手动操作）
- **禁止**修改 sdkconfig（用户手动通过 menuconfig 操作）
- **禁止**在没有 ESP-IDF 环境时静默跳过（必须报 environment-missing）

### 不该碰
- **不触碰** `$IDF_PATH` 下的 ESP-IDF 框架文件
- **不触碰** sdkconfig 和分区表 CSV 文件：只读取，不修改
- **不触碰** 非 ESP-IDF 环境的系统 Python 包

## 交接关系

- 当下一步意图是给硬件烧录程序时，将成功构建结果交给 `tool-flash-esp-idf`。
