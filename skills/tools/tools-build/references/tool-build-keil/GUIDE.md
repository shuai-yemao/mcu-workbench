...
```bash
python scripts/keil_builder.py --project app.uvprojx --uv4-path "<UV4_PATH>"
```

### Step 4 编译后查看产物
编译成功后在输出目录查找 .hex/.axf 文件，供 `tool-flash-keil` 使用。
...
### 不该激活
- 用户需要 IAR 编译 → 使用 `tool-build-iar`
- 用户需要 CMake + GCC 编译 → 使用 `tool-build-cmake`
- 用户需要 PlatformIO 编译 → 使用 `tool-build-platformio`
- 用户需要 ESP-IDF 编译 → 使用 `tool-build-esp-idf`
- 用户只需要烧录不需编译 → 使用 `tool-flash-keil`
...
- 下游：`tool-flash-keil`（编译 → 烧录）
- 辅助：`quality-map-analysis`（编译后分析 .map 文件）
- 报错时：`platform-cortex-registers` / `platform-peripheral-registers`（编译不报错但运行异常时查寄存器）
...
| `scripts/keil_map_analyzer.py` | 由 `quality-map-analysis` skill 提供 |
