...
description: 当需要通过 OpenOCD 启动或附着 GDB 会话，调用自带脚本完成固件下载、在线调试或崩溃现场检查时使用。当用户提到 GDB 调试、OpenOCD 调试、在线调试、断点调试、崩溃分析、core dump、单步调试、JTAG/SWD 调试时使用。
version: "1.0.0"
---
...
- 探针由脚本自动探测，优先级与 `tool-flash-openocd` 保持一致。
...
- 当复位后或继续运行后下一步是观察运行行为时，推荐 `observability-serial-monitor`。

## 边界定义

### 不该激活
- 用户的 PlatformIO 工程中有 `debug_tool` 配置 → 优先用 `debug-platformio`
- 用户只需要烧录（不需要 GDB 调试）→ 使用对应的 flash skill
- 用户没有 .elf 文件且无法从 HEX/BIN 反推符号
- 用户的调试探针不支持 OpenOCD（如某些专有调试器）

### 不该做
- **禁止**在没有 .elf（符号信息）时执行 GDB 调试（必须阻塞并提示用户提供）
- **禁止**修改用户固件（attach-only 模式不下载，crash-context 模式只读寄存器）
- **禁止**在 crash-context 模式中执行 `continue`（可能破坏崩溃现场）
- **禁止**在不确定 OpenOCD 配置时启动调试会话

### 不该碰
- **不触碰** Flash 内容（attach-only 和 crash-context 模式下）
- **不触碰** GDB 初始化脚本（~/.gdbinit）：使用临时生成的脚本
- **不触碰** 非调试目标的系统进程和线程

## 交接关系

- 当目标恢复运行后，需要继续观察运行期日志时，将成功会话交给 `observability-serial-monitor`。