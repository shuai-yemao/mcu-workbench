---
name: observability-rtt-porting
...
- **不覆盖 EasyLogger 上层封装** — 那是 `observability-elog` 的范畴
- **不覆盖 UART 输出** — RTT 和 UART 是互补方案，非替代
- 与 `observability-elog` 配合：RTT 作为 elog 的输出后端
- 用户只需要普通串口日志查看 → 使用 `observability-serial-monitor`
...
- **需要 JLink 调试技巧** → 使用 `tool-flash-jlink` / `debug-gdb-openocd` skill
- **需要日志分级管理** → 使用 `observability-elog` skill 对接 RTT 后端
- **RTT 日志发现异常后深入分析** → `debug-rtos` / `debug-crash-backtrace`
