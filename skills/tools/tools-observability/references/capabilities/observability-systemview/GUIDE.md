...
- **SystemView**: 在系统 PATH 中可执行，或通过 `<SYSTEMVIEW_PATH>` 显式指定。
...
从 [SEGGER GitHub](https://github.com/SEGGERMicro/SystemView) 或本机 SystemView 安装目录的 `Src/` 子目录复制以下文件到工程：
...
| 相邻 skill | 关系 |
|-----------|------|
| `observability-rtt-porting` | SystemView 依赖 RTT 传输通道，该 skill 负责 RTT 移植和缓冲区配置 |
| `debug-ozone` | Ozone 处理源码级调试和高级性能分析，SystemView 提供 RTOS 时序分析 |
| `rtos-freertos` | FreeRTOS + SystemView 集成时需理解 FreeRTOS 调度机制 |
| `debug-diagnostic-framework` | 诊断方法论中 SystemView 用于"状态机"层系统行为分析 |
