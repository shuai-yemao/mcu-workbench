...
- 成功后推荐 `observability-serial-monitor`（查看串口输出）或 `debug-platformio`（在线调试）。
- 失败时输出失败分类和日志证据。
...
- 用户只需要编译不烧录 → 使用 `tool-build-platformio`
...
- 从 `tool-build-platformio` 接收编译成功的工程信息。
- 烧录成功后交给 `observability-serial-monitor`（查看串口输出）或 `debug-platformio`（在线调试）。
