---
name: tool-flash-keil
description: 当需要通过 Keil MDK 烧录编译产物到目标 MCU 时使用。支持 .hex/.axf 文件通过 J-Link 烧录。与 tool-build-keil 配合形成完整的 Keil 编译→烧录工作流。当用户提到 Keil 烧录、MDK 下载、UV4 烧录、J-Link 配合 Keil、编译后烧录、tool-flash-keil 时使用。
version: "1.0.0"
---
...
与 `tool-build-keil` 配合形成完整的 Keil 编译→烧录工作流。

## 适用场景

- 已通过 `tool-build-keil` 完成编译，需要烧录产物到目标板验证
...
### Step 4 编译后直接烧录
```bash
python ../../../tools-build/references/tool-build-keil/scripts/keil_builder.py --project app.uvprojx --target "Release"
python scripts/keil_flasher.py --project app.uvprojx --target "Release"
```
...
| No hex/axf found | 工程未编译或输出路径错误 | 先运行 tool-build-keil 编译 |
...
### 不该激活
- 用户没有 J-Link 探针（使用 ST-Link/OpenOCD）→ 使用 `tool-flash-openocd`
- 用户使用 PlatformIO 平台 → 使用 `tool-flash-platformio`
- 用户使用 ESP-IDF 平台 → 使用 `tool-flash-esp-idf`
- 用户只需要编译不烧录 → 使用 `tool-build-keil`
...
- 上游：`tool-build-keil`（编译 → 烧录）
- 下游：`observability-serial-monitor` / `observability-rtt-monitor`（烧录后查看日志）
- 烧录失败时：`platform-option-bytes`（检查保护位）
