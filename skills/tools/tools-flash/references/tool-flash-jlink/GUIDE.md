...
### 不该激活
- 用户的目标板是 **ESP32/ESP8266** 系列 → 应使用 `tool-flash-esp-idf` / `tool-flash-platformio`
- 用户没有 J-Link 探针（如使用 ST-Link、DAP-Link、CMSIS-DAP）→ 应使用 `tool-flash-openocd`
- 用户明确提到 Keil MDK 的"Download"按钮 → 应使用 `tool-flash-keil`
...
- 上游：`tool-build-keil` / `tool-build-cmake` / `tool-build-platformio`（烧录前需先编译）
- 下游：`observability-serial-monitor` / `observability-rtt-monitor`（烧录后查看日志验证运行状态）
- 烧录失败时：`platform-option-bytes`（检查 RDP 保护位）
