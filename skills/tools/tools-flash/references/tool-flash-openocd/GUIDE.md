---
name: tool-flash-openocd
description: 当需要使用已探测或显式指定的产物与探针配置，调用自带脚本通过 OpenOCD 烧录嵌入式固件时使用。当用户提到 OpenOCD 烧录、openocd flash、ST-Link 烧录、DAP-Link 烧录、openocd 下载、探针烧录时使用。
version: "1.0.0"
---
...
- 根据用户意图推荐下一步 skill：做启动验证后推荐 `observability-serial-monitor`，需要调试时推荐 `debug-gdb-openocd`。

## 边界定义

### 不该激活
- 用户的目标芯片是 **ESP32/ESP8266** 系列 → 优先用 `tool-flash-esp-idf` / `tool-flash-platformio`
- 用户已有 Keil MDK 且工程已配置调试器 → 优先用 `tool-flash-keil`（使用工程内 Flash 算法）
- 用户明确说 "J-Link 烧录" → 优先用 `tool-flash-jlink`（JLinkExe 生态）
- 用户只有 .elf 文件但需要的是在线调试而非烧录 → 使用 `debug-gdb-openocd`

### 不该做
- **禁止**对 .bin 文件在无 `--base-address` 参数时静默使用默认地址
- **禁止**在探针未应答时反复重试（最多 3 次，每次间隔递增）
- **禁止**使用部分匹配的 OpenOCD 配置（interface 匹配但 target 不确定时，必须返回 ambiguous-context）
- **禁止**在未 halt 目标板时执行烧录

### 不该碰
- **不触碰** Option Bytes（系统配置区）：仅烧录 Flash 用户数据区
- **不触碰** 非当前探针连接的其他设备
- **不触碰** 固件源文件：只读取产物，不修改
