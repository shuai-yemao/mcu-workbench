...
description: 嵌入式 OTA 升级系统架构设计与开发指南。涵盖 OTA 系统架构（A/B 双区/单区+恢复/外置 Flash/双芯片）、OTA 状态机（Idle→Download→Verify→Apply→Rollback 五态设计）、固件下载协议与可靠性（HTTP/CoAP/MQTT/蓝牙/Ymodem/分块/续传/断点续传/超时重试）、双芯片 OTA 架构（WiFi MCU→主 MCU SPI 转发/共享 Flash/状态同步）、固件版本管理（语义版本/兼容性矩阵/强制升级/灰度升级）、升级安全（签名验证/加密传输/防回滚/防降级攻击）、回滚策略（自动回滚/手动回滚/出厂重置/数据分区保护）、STM32 OTA 实现详解（内部 Flash 双区/SDRAM 暂存/外部 QSPI Flash）、ESP32 OTA 实现详解（esp_https_ota/App Rollback/Factory Partition）、升级失败恢复（看门狗/Bad App Detection/Safe Mode）、带宽优化（差分/压缩/分段/增量）、量产 OTA 策略（批次/区域/设备组/条件升级）。与 release-ota-package（打包工具）互补——本 skill 专注 OT**A 系统的架构设计与策略决策**。当用户提到 OTA 系统设计、OTA 架构、空中升级、固件远程更新、远程升级系统、OTA 状态机、升级策略、A/B 分区 OTA、单区 OTA、差分 OTA、增量升级、升级回滚、OTA 安全、签名升级、加密升级、防回滚、灰度升级、静默升级、强制升级、批量升级、批量 OTA、省带宽 OTA、双芯片 OTA、ESP32 OTA、STM32 OTA、升级失败恢复、断点续传、升级进度、数据分区保护、AB 分区、OTA 分区表时使用。
version: "1.0.0"
---

# 嵌入式 OTA 升级系统架构设计指南

> 与 `release-ota-package`（OTA 固件包生成工具脚本）和 `system-bootloader`（Bootloader 引导跳转）互补——本 skill 覆盖 OTA **系统的架构设计、协议选型、状态机、安全策略和量产部署**。
...
### 不该激活
- 用户需要的是 OTA 固件包生成/打包（而非架构设计）→ 使用 `release-ota-package`
- 用户需要的是 Bootloader 引导跳转和 Flash 分区（而非 OTA 系统）→ 使用 `system-bootloader`
...
### 不该碰
- **不触碰** OTA 包的生成和打包（由 release-ota-package 负责）
- **不触碰** Bootloader 的 JumpToApp 实现
- **不触碰** 用户 OTA 服务器的部署配置

## 交接关系

- 上游：`system-bootloader`（Bootloader 引导/分区策略）
- 下游：`release-ota-package`（OTA 包生成和打包工具）
- 辅助：`security-firmware-signing`（固件签名/加密）
- 参考：`platform-internal-flash`（Flash 分区/Sector 操作）
