...
- Ymodem/EEPROM 场景中用户没有定义 EEPROM 存储布局或仅讨论 I2C 总线问题（应使用 bus-i2c skill）
...
- **禁止**生成签名 OTA 包时使用与 `security-firmware-signing` 不同的签名算法（防止 Bootloader 验签失败）
...
- **Ymodem 传输**：不负责串口物理层配置（波特率/流控/引脚），由 bus-uart skill 覆盖
- **双芯片架构**：不负责 W25Q64/AT24C02 的底层驱动实现，由 bus-spi / bus-i2c 和 bsp-device-adaptation skill 覆盖
...
- 上游：`security-firmware-signing`（签名/加密后的固件打包为 OTA）
- 上游：`tool-build-keil` / `tool-build-esp-idf`（编译产出新旧固件）
- 辅助：`system-bootloader`（Bootloader OTA 状态机集成设计）
- 实测：`observability-serial-monitor`（OTA 升级日志观察）/ `observability-rtt-monitor`（RTT 实时日志）
- 参考：`platform-option-bytes`（量产时配合 RDP1 保护 OTA 分区）
