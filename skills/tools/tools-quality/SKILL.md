...
命名检查必须区分代码来源：新生成的 `.c/.h` 文件严格执行
[`style-profile.md`](references/style-profile.md) 的完整命名规则；修改已有文件时保留已有文件名、公开 API
和无关区域，只检查本次修改的函数、代码块和新增符号。不得通过自动重命名或全文件重排消除既有命名基线。
