...
3. 除非项目或用户明确采用其他约定，否则按 [项目风格 profile](references/style-profile.md) 的硬性约束执行：强制 `{proj}_`（项目简称）文件命名、`g_/s_/p_/pf_` 变量前缀、全函数 Doxygen 注释、左对齐-填充-右对齐段注释，并将 80 列作为硬限制。
