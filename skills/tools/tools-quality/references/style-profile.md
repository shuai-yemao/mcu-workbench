...
| 10 | 同一数据类型定义或代码块的行尾注释起始列对齐 | `member_a; /**< 说明 */` | 行尾注释起始列分散 |
...
| 对象 | 规则 | 示例 |
|------|------|------|
| 文件 | `{proj}_{层}_{模块}_{功能}.h/.c` | `{proj}_platform_sensor.h` |
...
- 生成器使用三级阶段标签 `清理`、`事件`、`回调`、`转发`、`校验`、`状态`、`结果`、`处理`，每个标签从 `/*` 起占 40 列。
- 阶段注释说明逻辑责任和资源/并发/错误边界，不逐行翻译代码，也不得用注释掩盖未处理的错误路径。
- 生成器输出必须经过本规范的 `.clang-format`，并执行格式与注释门禁；生成结果缺少必需阶段注释时必须判定失败。

**已有代码文件重生成契约：**

- 已有 `.c/.h` 文件与新生成文件使用同一套注释、分区和代码块对齐管线。
- 既有注释的语义内容必须保留；旧的分区宽度、Doxygen 排版、缺失 API 标签、缺失类型/成员/阶段注释必须按本规范重生成。
- 同一 `typedef enum/struct` 或连续代码块内，声明、赋值、初始化符号以及行尾注释必须分别对齐；对齐不能跨越空行、分区注释或不同代码块。
- 任何重生成结果仍必须满足 80 列上限；无法同时满足时，必须拆分代码或注释，不得静默放弃强制对齐。
...
| 16 | 同一数据类型定义或代码块的行尾注释起始列对齐 | `member_a; /**< 说明 */` | 注释列不对齐 |
...
- [ ] 函数内每个非显然逻辑阶段都有三级分区注释（`清理`/`事件`/`回调`/`转发`/`校验`/`状态`/`结果`/`处理`按需出现，分区从 `/*` 起占 40 列）
