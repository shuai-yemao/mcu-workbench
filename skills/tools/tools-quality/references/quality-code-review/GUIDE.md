...
对插件生成代码和插件要求修改的代码，本层所有命名、函数、注释和格式条目都是硬门禁；任一条失败都必须阻塞。
既有无关代码只记录基线，不能豁免本次插件要求修改的范围。

| # | 检查项 | 规范要求 | 严重度 |
|---|--------|---------|--------|
| 2.1 | 使用 TAB 缩进并保持连续声明组列对齐 | 编码规范 #1 | **CRITICAL** |
| 2.2 | 代码和注释不得超过 80 列 | 编码规范 #2 | **HIGH** |
| 2.3 | 插件生成或插件要求修改的文件命名 `<layer>_<module>_<role_or_feature>.h/.c`；仅既有无关文件名保留 | 命名规范 | **CRITICAL** |
...
| 2.9a | 公共 `.h` 函数声明和核心 `.c` 函数有 `@brief` 且说清函数意图 | 编码规范 #6 | **HIGH** |
| 2.9b | 公共 `.h` 函数声明和核心 `.c` 函数的 `@param` 标注所有参数，方向 `[in]`/`[out]`/`[in/out]` 正确 | 编码规范 #6 | **HIGH** |
| 2.9c | 公共 `.h` 函数声明和核心 `.c` 函数的 `@retval` 覆盖所有可能返回值 | 编码规范 #6 | **HIGH** |
| 2.9g | 简单且自解释的普通 `.c` 函数不机械添加完整 Doxygen；仅在存在非显然语义时补充必要说明 | 编码规范 #6 | **MEDIUM** |
| 2.9h | 相似 `.c` 函数不重复复制相同注释段落，只保留代表性说明和各函数独有差异 | 编码规范 #6 | **MEDIUM** |
...
| 2.10 | 核心 `.c` 函数只对非显然硬件、资源、并发、错误恢复或算法约束添加 `/* ... */` 说明，并按连续注释组对齐起始列和结束列；普通函数不机械添加 | 编码规范 #7 | **MEDIUM** |
| 2.11 | 头文件组织顺序正确 | header-template 顺序表 | **MEDIUM** |
| 2.12 | 枚举类型有 `@brief` 总述，枚举项有对齐的普通行尾注释 | 类型注释规范 | **MEDIUM** |
| 2.13 | 结构体类型有 `@brief` 总述，成员有对齐的普通行尾注释 | 类型注释规范 | **MEDIUM** |
