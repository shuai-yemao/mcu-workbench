...
description: 嵌入式 C 编码规范 — 命名规则、头文件模板、函数结构、Doxygen 注释、宏/类型注释、代码排版、九大核心原则。当用户要求写嵌入式 C 代码、STM32 驱动、BSP 层、传感器驱动时必须加载此技能作为编码约束。
version: "1.0.0"
...
> 本技能整合命名规范 + 头文件模板 + Doxygen 注释规范 + 代码排版规则 + 九大核心原则。
...
- ISR 与 Task 共享变量声明为 `volatile`，并通过临界区保护
