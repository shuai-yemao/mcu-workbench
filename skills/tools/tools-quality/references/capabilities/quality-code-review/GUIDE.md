...
description: 嵌入式项目代码审查 — 九层检查（语法/规范/逻辑/安全/内存/中断/初始化/错误/反模式），对照 style-profile 审查生成、修改和已有代码。
version: "1.0.0"
---
...
| 2.3 | 新文件命名 `<layer>_<module>_<role_or_feature>.h/.c`；已有文件名保留 | 命名规范 | **MEDIUM** |
| 2.4 | 对外函数命名 `<layer>_<module>_<action>()`，内部 static 函数 `<module>_<action>()` | 命名规范 | **MEDIUM** |
...
| 2.8a | 新文件不得使用 `_wrapper`、`_model`、`_state`、`_fault_code` 后缀；允许 `_driver`、`_handle`、`_port`、`_config` | 命名规范 | **HIGH** |
| 2.8b | 名称可读、可搜索、使用领域词汇；同一概念只使用一个稳定词汇 | 命名规范 | **HIGH** |
| 2.8c | 新增名称必须带有必要语境并准确描述职责，不得使用双关语或无意义泛化词 | 命名规范 | **HIGH** |
| 2.8d | 函数短小，只保留当前职责所需语句 | 函数设计规范 | **HIGH** |
| 2.8e | 函数只做一件事，语句处于同一抽象层级 | 函数设计规范 | **CRITICAL** |
| 2.8f | 函数按由高到低的抽象顺序排列，调用者后面紧跟下一层函数 | 函数设计规范 | **HIGH** |
| 2.8g | 参数数量以 0 个为理想目标，通常不超过 1 个；2 个及以上必须说明理由 | 函数设计规范 | **HIGH** |
| 2.8h | 相同业务规则、校验、转换和错误处理不得重复实现 | 函数设计规范 | **HIGH** |
...
| # | 检查项 | 方法 | 严重度 |
|---|--------|------|--------|
| 3.1 | **按有效性契约判定指针**：外部可为空边界判空，已验证内部指针不重复机械判空 | 核对 nullability、验证边界、前置条件和错误路径 | **CRITICAL** |
| 3.1a | 指针与长度、生命周期、所有权、对齐和异步借用期限一致 | 核对接口契约和调用链 | **CRITICAL** |
...
| 5.3 | 指针有效性、生命周期、所有权和长度关系已证明；判空不能替代内存安全证明 | 核对边界、所有权和生命周期证据 | **CRITICAL** |
