...
MISRA 规则元数据位于 [`../capabilities/quality-static-analysis/misra-rule-metadata.json`](../capabilities/quality-static-analysis/misra-rule-metadata.json)，当前覆盖
...
- 上游：`tool-build-keil` / `tool-build-cmake`（编译通过后自动触发静态分析）
- 互补：`coding-standards`（编码规范速查）+ `quality-code-review`（人工审查）
- 下游：修复问题后重新编译 → `tool-flash-jlink` / `tool-flash-keil` 烧录验证
