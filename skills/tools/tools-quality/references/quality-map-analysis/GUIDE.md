...
- 上游：`tool-build-keil` / `tool-build-cmake`（编译链接后自动触发 .map 分析）
- 下游：发现内存溢出 → `platform-stm32-hal` / `rtos-freertos`（优化代码体积）
- 辅助：`quality-static-analysis`（结合代码质量检查）
...
### 不该激活
- 用户需要的是固件本身的逻辑调试（而非内存布局分析）→ 使用 `debug-gdb-openocd` / `debug-rtos`
