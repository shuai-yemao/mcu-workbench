...
>
> 关联：`quality-map-analysis`（.map 文件分析）、`platform-sram`（SRAM 配置）、`platform-internal-flash`（Flash 配置）
...
- **不覆盖 .map 文件分析** — 那是 `quality-map-analysis` skill
- **不覆盖内存架构理论** — 那是 `platform-cortex-memory` skill
- **不覆盖 Flash/SRAM 外设配置** — 那是 `platform-internal-flash` / `platform-sram`
- **不覆盖 Bootloader 分区设计** — 那是 `system-bootloader`
