...
最小产物包括：APP 层目录图、启动链、组件职责表、跨层依赖审计、测试入口和未决风险。需要完整分层证据时交接 [`software-architecture-knowledge-graph.md`](../../workflow/workflow-review-gate/references/software-architecture-knowledge-graph.md)。

## GR5526 交接

验收 `Src/app/main.c`、`manager/`、`task/`、`ux_logic/` 与 `lv_user_task_create()`；OS 并发接口交给 [`os-adapter`](../../platform/platform_os/SKILL.md)，界面能力交给 [`vendor_lvgl`](../../vendor/vendor_lvgl/SKILL.md)。
