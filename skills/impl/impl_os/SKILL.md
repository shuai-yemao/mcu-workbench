...
Impl 先保持 OSAL 的超时单位、所有权和错误语义，再映射到 `xQueue*`、`xTask*` 等原生 API。Tickless、Idle Hook 和 Trace Hook 属于本层；任务看门狗、故障恢复和系统重启策略交给 [`software-system`](../../system/software-system/SKILL.md)（阶段 4 升格 service_system）。
