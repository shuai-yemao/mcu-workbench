...
description: Platform 中间件 API：每个中间件默认只提供一个稳定公共头和固定后端实现；LVGL 以唯一 platform_gui.h 暴露契约，具体 API 由 Impl Adapter 落地，不绑定 Vendor/HAL/RTOS。
---
...
LVGL 接入 `platform_middleware` 时，Platform 侧只保留一个公共头：

```text
03_Platform/platform_middleware/platform_gui.h
```

`platform_gui.h` 是 GUI 能力的唯一公共入口，不再生成 `platform_lvgl.h`、
`platform_gui_config.h` 或其他 Vendor 命名的公共头。该头只声明与 Vendor 无关的
`platform_gui_*()` 能力，例如生命周期、Tick/Process、显示 Flush 完成和输入读取；
具体函数集合必须以目标工程的 LVGL 版本、显示/输入设备和 OS 证据锁定。

Platform 公共头不得出现 `lvgl.h`、`lv_display_t`、`lv_indev_t`、LVGL 枚举、
`lv_conf.h` 或 Vendor 配置结构。分辨率、颜色格式、绘制缓冲、Flush/Input 回调、
Tick、锁和 Owner Task 都由 Impl/工程配置承载。

LVGL 采用“单头、Impl 直实现”profile：Platform Middleware 目录对外只保留
`platform_gui.h`，`impl_lvgl_gui.c/.h` 对其中声明的 `platform_gui_*()` 符号提供唯一
实现并完成 `platform_err_t` 映射。除非目标工程有稳定转发、状态或构建隔离的真实证据，
不在 Platform 目录增加 `platform_gui.c`；若确需增加，必须在 RCP/Review Gate 中单独放行，
且不得产生第二套 GUI 契约入口。

调用链固定为：

```text
Service → platform_gui.h → impl_lvgl_gui.c → LVGL Vendor
                                      ├→ 显示/输入 Port
                                      └→ Tick/锁/Owner Task Port
```

显示/输入的硬件绑定可以在 Impl 内部使用已确认的 `platform_bsp` 能力，
Tick/锁/任务可以在 Impl 内部使用已确认的 `platform_os` 能力；这些 Platform 头、
原生 RTOS 类型和具体设备句柄不得回流到 `platform_gui.h`。
...
`03_Platform/platform_middleware/` 当前按扁平目录组织能力文件，默认每个能力域只有一个公共头和一个稳定后端入口。
普通固定转发 profile 可由 Platform `.c` include 对应 Impl Adapter 头；LVGL 使用上面的“单头、Impl 直实现”
profile。无论哪种 profile，公共头都只暴露 `platform_*_*()` API，不得暴露或保存 Vendor、HAL、RTOS 或
Impl 私有类型。Middleware 不建立 Ops/context 注册对象。
...
- [ ] LVGL 接入只保留 `platform_gui.h`，没有 `platform_lvgl.h`、第二个配置头或第二套 GUI 符号；
...
- [ ] LVGL 的具体实现、版本 API、显示/输入回调、Tick、锁和缓冲区均位于 Impl/工程配置；
