...
- **统一错误码**：`platform_error.h` 全局错误码基线（PLATFORM_OK / ERR_PARAM / ERR_TIMEOUT / ERR_BUSY / ERR_NOT_SUPPORTED / ERR_IO / ERR_NO_MEM / ERR_STATE / ERR_CRC），Service/Impl 只在其后扩展，禁止重复编号；
...
Platform IIC 公共接口提供初始化、释放、事务 read/write、memory read/write、可选 DMA/IRQ 启动、取消、状态码和带明确单位的超时；它不是只有读写函数的薄封装。Platform 只定义 `iic_bus_ops_t` 与硬件/软件后端**接口**，不拥有锁、任务、业务缓存或设备协议；锁与回调上下文属于 Handler。Impl 可持有平台对象并选择后端，但不能包含器件协议或把自身变成软件时序后端。START、STOP、ACK、逐字节发送、SDA 方向和位时序只属于软件后端私有实现（Impl）。
...
交接：厂商 HAL/LL/CMSIS 交给 [`vendor_stm32`](../../vendor/vendor_stm32/SKILL.md)，板上器件流程交给 [`bsp-hal-driver`](../../bsp/bsp-hal-driver/SKILL.md)（Impl 层阶段 3 落地后归 impl_bsp）。
