...
Platform 定义能力契约，**不绑定芯片**；技能目录允许无芯片/RTOS/厂商依赖的公共实现（如平台侧公共工具），绑定芯片的实现必须在 Impl。本技能面向 MCU 内置资源的能力接口：时钟、引脚、外设实例、中断/DMA、总线契约和后端算法接口。它调用厂商 Driver 的原生接口由 Impl 承接，不创建 Adapter，也不实现外部器件协议。
