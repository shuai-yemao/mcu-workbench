...
> 与 vendor_mcu（HAL API 参考 + DMA+IDLE 诊断）互补：本 skill 覆盖 UART 独有的
...
- 硬件流控（RTS/CTS）
- 半双工单线模式
- DMA 双缓冲 + IDLE 变长接收（参考 vendor_mcu 诊断）
- Printf 重定向（四种方式对比）
...
DMA+IDLE 变长接收的详细诊断参考 `platform-stm32-hal` 的 `references/troubleshooting-guide.md` 第 6b 节。
本 skill 只补充关键点：
...
**诊断和修复**：切换到 `platform-stm32-hal`，查阅其 `references/troubleshooting-guide.md` 的 DMA+IDLE 专节。
...
各 MCU 平台 UART 操作方式的等效映射见 `platform-mcu-architecture`（MCU 架构对比中央参考）。
