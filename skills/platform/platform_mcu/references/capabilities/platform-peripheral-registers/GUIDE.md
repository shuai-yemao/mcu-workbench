---
name: platform-peripheral-registers
...
> 与 platform-cortex-registers（内核寄存器）互补：本 skill 覆盖 MCU 芯片厂商设计的外设寄存器。
...
void spi_write_read(SPI_TypeDef *spi, uint8_t *tx, uint8_t *rx, uint16_t len)
{
    ...
    // 注意：BSY 在从机模式下不可靠（见 bus-spi skill）
}
```
...
### 不该激活
- 用户需要的是 HAL 层 API 调用 → 使用 `platform-stm32-hal`
...
- 上游：`platform-stm32-hal`（HAL 对寄存器做了封装，本 skill 教你直读）
