...
```c
// 参见 bus-uart Skill + platform-stm32-hal 6b
...
- **不重复 peripheral-adc/bus-uart/peripheral-timer 中外设+DMA 的应用层代码**，仅覆盖 DMA 架构和配置原则
...
- 上游：`platform-stm32-hal`（HAL 层 DMA 初始化规范）
