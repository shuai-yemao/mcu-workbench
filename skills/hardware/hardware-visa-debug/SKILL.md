...
### 不该激活
- 用户没有 VISA 仪器（示波器、万用表、信号源等），只是用串口调试 MCU → `observability-serial-monitor`
- 用户需要在 MCU 资源受限环境中实现 SCPI 解析 → `bsp-device-adaptation`
...
- 从 `tool-build-keil` / `tool-build-platformio` 烧录固件后，用此 skill 验证硬件输出信号。
- 与 `observability-serial-monitor` 互补：observability-serial-monitor 查看串口调试输出，hardware-visa-debug 进行仪器级测量验证。
- 与 `protocol-modbus` / `protocol-can` 互补：协议调试配合仪器测量。
