---
name: service_sensor
description: Service 业务服务：多传感器汇聚、滤波、单位转换、上报策略（带业务策略，非驱动封装）。
---
...
Service 是 **App 常见业务抽象**（D10）：把产品业务中可沉淀、可复用的能力组织成服务，**带业务策略**，不是驱动的简单封装，也不是中间件封装。本服务只依赖 Platform 接口（platform_bsp(imu/airpressure/temperature_humi/…)），不 include Vendor / Impl / HAL 任何符号。

## 业务策略

多传感器采样编排、滤波/校准、单位转换、异常值剔除、周期上报。
