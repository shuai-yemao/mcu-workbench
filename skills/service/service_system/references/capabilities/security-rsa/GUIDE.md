...
**建议**：新项目优先选 ECDSA P-256（`security-firmware-signing` skill 默认使用）。
...
- **不覆盖** ECDSA/Ed25519 签名算法 → 使用 `security-firmware-signing`
...
- 上游：`security-firmware-signing`（固件签名/验签的主入口，整合 RSA/ECDSA）
- 上游：`platform-option-bytes`（签名后配置 RDP/PCROP 保护密钥）
- 互补：`security-aes`（RSA + AES 混合加密：RSA 加密 AES Key）
- 参考：`platform-mcu-architecture`（硬件 RSA 加速选型）
