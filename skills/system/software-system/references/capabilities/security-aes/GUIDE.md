...
- **不覆盖** RSA/ECC 非对称加密 → 使用 `security-rsa`
- **不覆盖** 哈希算法（SHA256/MD5）→ 使用 `security-crc`（CRC 部分）或 mbedTLS
- **不覆盖** TLS/SSL 协议栈 → 使用 `protocol-mqtt`（MQTT over TLS）
- 密钥安全是系统工程，硬件 AES 不解决密钥泄露问题

## 交接关系

- 上游：`security-firmware-signing`（固件签名+加密组合使用）
- 上游：`release-ota-package`（OTA 包 AES 加密）
- 互补：`security-rsa`（AES+RSA 混合加密：RSA 加密 AES Key）
- 互补：`security-crc`（CRC 完整性校验 + AES 加密双重保障）
- 参考：`platform-mcu-architecture`（CRYP 硬件支持选型）
