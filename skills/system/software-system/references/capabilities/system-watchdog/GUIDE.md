...
> 与 `platform-cortex-interrupts`（NMI/异常架构）互补：本 skill 覆盖 IWDG/WWDG 独有的时钟计算、窗口策略、喂狗设计和故障排查。
> 与 `system-bootloader`（看门狗回滚）互补：本 skill 覆盖看门户外设本身的配置与调试。
...
**配置方法**（通过 `platform-option-bytes` skill）：

```bash
# 使用 J-Link Commander 设置 Option Bytes
# 参考 platform-option-bytes skill 文档
```
...
### 不该激活
- 用户需要的是独立于看门狗的系统复位原因分析 → 使用 `platform-cortex-registers`（RCC_CSR）
- 用户需要的是系统级死锁恢复方案（而非看门狗配置）→ 使用 `debug-diagnostic-framework`
...
详见 `platform-mcu-architecture`（MCU 芯片架构与开发方式对比）。
...
## 交接关系
- 下游：`release-ota-update`（升级失败看门狗恢复）
- 同层：`peripheral-timer` / `peripheral-adc`（同为外设配置+调试类 Skill）
- 调试时：`debug-diagnostic-framework`（喂狗复位诊断）
- RTOS 配合：`rtos-freertos`（任务级喂狗 + tickless idle）
