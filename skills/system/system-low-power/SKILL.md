...
> 与 `system-watchdog`（低功耗下看门狗行为）互补：本 skill 覆盖完整的功耗模式决策树、电源架构和唤醒策略。
> 与 `platform-sram`（SRAM 保持策略）互补：本 skill 覆盖不同功耗模式下的内存保持配置。
...
## 交接关系
- 同层：`platform-peripheral-registers`（RCC 时钟门控、PWR 寄存器）
- 同层：`peripheral-timer` / `bus-uart`（LPTIM / LPUART 外设配置）
- 下游：`debug-rtos`（tickless idle 调试）
- 调试时：`debug-diagnostic-framework`（低功耗异常排查）
