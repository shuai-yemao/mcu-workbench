...
> 与 `platform-internal-flash`（Flash 编程/分区/Sector 操作）和 `release-ota-update`（OTA 架构与状态机）互补——本 skill 专注 Bootloader **架构设计、启动流程、引导决策、安全链和调试**。
...
### 不该激活
- 用户只需要 Flash 扇区写入/擦除操作（非 Bootloader 设计）→ 使用 `platform-internal-flash`
...
- 上游：`platform-internal-flash`（Flash 编程/分区操作基础）
- 下游：`release-ota-update`（OTA 架构设计）
- 辅助：`platform-option-bytes`（RDP/WRP 保护配置）
- 验证：`system-watchdog`（看门狗回退机制）
