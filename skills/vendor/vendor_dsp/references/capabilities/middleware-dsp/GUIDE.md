...
- **不覆盖** FFT（快速傅里叶变换）→ 使用 `middleware-fft`
...
- 互补：`middleware-fft`（频率域分析，与 FIR/IIR 时域滤波互补）
- 下游：`peripheral-adc`（DSP 输入端：ADC 采样数据）
- 下游：`peripheral-motor-control`（PID 控制器、电流环/速度环 DSP）
- 参考：`platform-mcu-architecture`（DSP 指令集支持选型）
