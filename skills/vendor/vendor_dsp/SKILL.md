...
交接：公共算法不触碰设备；任何 Middleware Port 获取设备能力只能经 BSP Wrapper（[`bsp-wrapper`](../../bsp/bsp-wrapper/SKILL.md)），调度、任务和等待只能经 [`os-adapter`](../../os/os-adapter/SKILL.md) 提供的 OS Wrapper。`core-mcu` 只承载底层 MCU 公共能力，不是 Middleware 的设备直连入口。
