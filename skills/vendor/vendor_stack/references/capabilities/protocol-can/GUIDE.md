...
- 用户只需要串口日志 → `observability-serial-monitor`
- 用户需要在 MCU 固件侧实现 CAN 驱动而非调试 → `bsp-device-adaptation`
...
- 从 `tool-build-keil` / `tool-build-platformio` 烧录固件后，用此 skill 验证 CAN 通信。
- 与 `observability-serial-monitor` 互补：observability-serial-monitor 查看串口调试输出，protocol-can 进行 CAN 协议级调试。
