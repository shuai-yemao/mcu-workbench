...
- 上游：`bus-uart`（NMEA 数据 UART 接收，DMA+IDLE 变长）
- 互补：`protocol-cellular`（AGPS 辅助数据通过蜂窝网络下载）
- 互补：`system-low-power`（GNSS 低功耗模式管理）
- 参考：`platform-mcu-architecture`（GNSS 模块选型）
