...
- 下层：`protocol-wifi`（MQTT over WiFi 典型方案）
- 下层：`protocol-cellular`（MQTT over 4G/NB-IoT 方案）
- 下层：`bus-uart`（AT 命令 MQTT 模块底层）
- 互补：`protocol-ble`（BLE + MQTT 网关桥接）
- 参考：`platform-mcu-architecture`（MQTT 方案选型）
