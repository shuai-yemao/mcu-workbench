...
- 用户只需要局域网 TCP/IP 通信（已有以太网/WiFi）→ 使用 `protocol-mqtt` 或 TCP socket
- 用户没有 SIM 卡或蜂窝模块硬件
...
- 上游：`bus-uart`（AT 命令模块底层串口通信）
- 上层：`protocol-mqtt`（蜂窝网络上的 MQTT 应用协议）
- 对比：`protocol-wifi`（蜂窝 vs WiFi 方案选型对比）
- 对比：`protocol-lora`（NB-IoT vs LoRa 对比）
- 参考：`platform-mcu-architecture`（蜂窝芯片/模块选型）
