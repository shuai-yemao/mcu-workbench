...
description: 当用户要为嵌入式固件工程在创建起始阶段设计/引导，或后续初始化、扫描、更新、审计、校验 Claude Code 多文件规则（Claude.md、.claude/rules、规则漂移、分层规则）时使用。存量工程先扫描真实证据；新工程先设计并确认规则，再生成 Claude 文件。README 和项目文档上下文交给 workflow-document-context。
---
...
为目标固件工程维护受管的 Claude 宿主规则：创建阶段生成通用根 `Claude.md` 和五层规则，并将项目实际证据保存到分层扫描快照和架构报告。
...
- `design`：只读输出规则 profile、依赖方向、Claude 文件清单和创建顺序；新工程必须先执行。
- `bootstrap`：项目创建起始阶段的写入入口；没有规则确认时返回 `RULES_NOT_CONFIRMED`，只有追加
  `--rules-confirmed --write` 才能生成通用根 `Claude.md`、通用五层规则和配置/状态文件。
- `scan`：只读扫描 CMake、CubeMX、FreeRTOS 和 C/C++ include 证据。
- `init`：建立首次 Claude 规则配置与受管产物计划。
- `sync`：依据已有配置重新扫描并更新架构报告和状态快照；不把项目事实写回通用 Claude 文件和五层规则。
- `validate`：检查 Claude 分层扫描快照、受管文件和现有架构契约；README 由 `workflow-document-context` 校验。
...
README 管理范围、受管区块、配置和状态由 `workflow-document-context` 负责；本 Skill 不再拥有 README 文件。
...
| 项目根及文档上下文范围内目录的 `README.md` | `workflow-document-context` | 本 Skill 不读取后重写，不创建或删除。 |
...
design（只读规则方案）
  ...
  → 生成项目目录、源码和构建配置
  → sync --write（更新架构报告和状态快照，不改写通用规则）
...
2. 对首次接入执行 `init` 预览；检查根文件保留内容、规则目录和配置覆盖是否正确。README 另由 `workflow-document-context` 处理。
3. 在用户确认差异后运行 `init --write` 或 `sync --write`。
4. 运行 `validate`；它必须报告规则漂移、架构静态错误、未确认项，以及旧 layout 键的迁移提示（如有）。README 缺失和受管区块漂移由 `workflow-document-context` 报告。
...
Claude scope: <managed rule files, configuration, architecture report>
