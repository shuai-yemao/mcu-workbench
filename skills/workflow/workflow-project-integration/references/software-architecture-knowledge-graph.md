...
flowchart TD
    ...
    BW --> BP
    BP -->|constructs and injects| BD
    BP -->|constructs and registers| BH
    BH -->|Driver Ops / pf_*| BD
    BD --> CORE
    CORE --> DRIVER
...
3. Adapter 只出现在 OS 和 BSP：OS 是 Wrapper → Port；BSP 对外是 Wrapper → Port，Port 在内部构造 Driver/Handle 并注册 Driver Ops，Handle 再调用 Driver 实例 `pf_*`。
