---
name: workflow-code-porting
description: "跨 MCU、SDK 或工具链的代码移植"
---
...
| 2 | DMA 传输不完成 | F4 stream 模型和 F1 channel 映射不同 | 参考 `peripheral-dma` skill 重配 | 2h |
...
- 上层：`workflow-architecture`（分层架构参考）
- 下层：`platform-stm32-hal`（目标芯片 HAL 用法）
- 辅助：`peripheral-dma`、`peripheral-timer`、`bus-uart` 等外设 skill（逐外设对照）
- 验证：`tool-build-cmake` / `tool-build-keil`（编译验证）+ `tool-flash-jlink`（烧录验证）
- 调试：`debug-gdb-openocd`（逐层调试）+ `platform-cortex-registers`（HardFault 分析）
