...
| 技能 | 对应层 | 说明 |
|------|--------|------|
| `platform-stm32-hal` | Driver | HAL 库使用指南 |
| `platform-peripheral-registers` | Driver | 寄存器级操作 |
| `platform-cortex-registers` | Driver | Cortex-M 内核寄存器 |
| `bus-i2c` / `bus-spi` / `bus-uart` 等 | Driver + Core | 总线协议 + MCU 外设封装 |
| `bsp-device-adaptation` | BSP | 板级外设驱动 |
| `rtos-freertos` | OS | RTOS 内核 |
| `middleware-fatfs` | Middlewares | 文件系统中间件 |
| `workflow-code-porting` | 跨层 | 涉及 Driver/Core/BSP 三层的修改 |
| `workflow-architecture` | 全层 | 所有层的架构决策 |
