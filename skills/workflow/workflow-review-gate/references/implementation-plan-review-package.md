...
> 本包由 `workflow-review-gate` 在代码前审查阶段产出，是 `lightweight/full` 请求的四张清单审计记录；审查放行后，按风险力度整合出的 `spec.md` 是下游 Skill 的正式需求/约束输入。`workflow-integration-plan` 后续生成的 `plan.md` 是经过方案审查的实施计划输入。四张清单（工程现状表、文件施工清单、代码生成约束清单、验收测试清单）只保留在本包内，不分别输出到实际工程。`prototype` 只在 RCP 中记录目标、临时边界、有效期和不承诺项，不生成本包和正式 `spec.md`。不得以未引用工程证据的方案文字代替本包，也不得写入插件仓库内部。

## 文档落盘前置条件

- `项目路径与提交` 必须记录用户明确提供或 Router 已确认的目标项目绝对路径。
- 正式项目输出文件的根目录固定为 `<project_root>/00_Docs/04_需求文档/`；`lightweight/full` 门禁的下游正式文件为 `spec.md`。四张清单只存在于 `Review-Package` 章节中，不生成独立项目文件。
...
`lightweight/full` 请求的四张清单必须保留在本 `Review-Package` 中，作为四个审计章节，并在审查放行后按力度整合为下游正式输入 `spec.md`。不得将它们分别输出为四个 Markdown 文件：

| 审计章节 | 章节名称 | 必须包含 |
|---|---|---|
| 工程现状表 | `## 1. 工程现状表` | 工程事实、证据位置、可信等级、影响范围、待确认项 |
| 文件施工清单 | `## 2. 文件施工清单` | 新增/修改/不建议动、所属层、施工理由、生成边界、责任 Agent、状态 |
| 代码生成约束清单 | `## 3. 代码生成约束清单` | 接口/依赖、禁止跨层调用、并发/ISR、生成文件边界、未决约束 |
| 验收测试清单 | `## 4. 验收测试清单` | 证据等级、验收项、命令/条件、预期结果、责任 Agent、状态、产物/阻塞 |
...
四个章节必须与 `Review-Package` 同步更新；任何事实等级变化、新证据、阻塞关闭或验收状态变化，都必须同步回写对应章节和本包。目标项目路径缺失、歧义或不可访问时，不生成 `spec.md`，状态保持 `待补证/阻塞`。

## 6. 下游 `spec.md` 整合输出

对 `lightweight/full` 请求，四张清单审查完成后必须将其整合为目标项目文档目录下的固定文件：
...
`spec.md` 只能由四张清单整合生成，不得引入清单之外的新需求或未标记的推测。`lightweight` 只保留范围、非目标、3–5 条验收标准、风险和回滚；`full` 保留完整约束、计划交接和验证证据。门禁阻塞时可以生成状态为 `阻塞` 的审查草稿，但不得交给下游 Skill；只有门禁放行后，`spec.md` 才能作为正式施工输入。`prototype` 不生成占位 `spec.md`。任何新事实或事实等级变化都必须先回写四张清单和本 `Review-Package`，再更新 `spec.md`。
