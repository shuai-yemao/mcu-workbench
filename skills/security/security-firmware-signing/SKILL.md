---
name: security-firmware-signing
...
- 用户需要的是 AES 加密封装格式的完整 OTA 包（应使用 release-ota-package skill）
...
- **仅加密模式**：不负责将加密固件通过 Ymodem/UART 传输的集成（由 release-ota-package 覆盖）
...
- 上游：`tool-build-keil` / `tool-build-cmake`（编译出原始 .bin 后签名）
- 下游：`release-ota-package`（签名后的固件打包为 OTA 升级包）
- 辅助：`platform-option-bytes`（签名后配合 RDP 保护使用）
