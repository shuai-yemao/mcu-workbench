...
- **不覆盖** 数据加密 → 使用 `security-aes`
- CRC 不是加密哈希，不能用于安全校验（CRC 可被逆向构造碰撞）

## 交接关系

- 上游：`protocol-ymodem`（Ymodem 帧 CRC 校验）
- 上游：`release-ota-package`（OTA 包头部 CRC32 校验）
- 互补：`security-aes`（CRC 完整性 + AES 加密安全组合）
- 互补：`system-bootloader`（Bootloader 中的固件 CRC 校验）
