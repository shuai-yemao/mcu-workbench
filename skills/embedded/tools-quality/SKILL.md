...
代码审查、Map 分析、静态分析、格式检查和 Unity 的原始资料分别保留在 `references/quality-*/GUIDE.md` 下；需要脚本时使用对应命名空间中的脚本。

格式检查基于 `.clang-format` 配置，使用 `clang-format --dry-run --Werror` 或 `clang-format -i` 自动修复。
