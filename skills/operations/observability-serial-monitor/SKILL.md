---
name: observability-serial-monitor
description: 当需要识别正确串口、调用自带脚本抓取日志，并分析嵌入式固件运行状态时使用。当用户提到串口、串口监控、串口日志、UART 日志、COM 口、波特率、串口调试、putty、串口助手、查看日志时使用。
version: "1.0.0"
---
...
- Linux、macOS、Windows 的串口命名规则应在目标环境中探测，不依赖个人目录或本机固定路径。
...
### 不该激活
- 用户使用的是 SEGGER RTT 而非 UART 日志 → 使用 `observability-rtt-monitor`
- 用户需要的是 CAN 总线数据而非串口日志 → 使用 `protocol-can`
- 用户需要的是 Modbus RTU 协议调试 → 使用 `protocol-modbus`
- 用户需要的是 VISA 仪器通信（SCPI）→ 使用 `hardware-visa-debug`

### 不该做
- **禁止**在无串口设备时静默跳过（必须报 connection-failure 或 ambiguous-context）
- **禁止**在多串口候选时自动选择（必须列出并让用户确认）
- **禁止**向串口写入数据（除非用户明确要求交互式通信）
- **禁止**修改串口设备驱动或系统端口配置

### 不该碰
- **不触碰**非目标串口的其他串口设备
- **不触碰**系统级串口驱动和注册表/udev 规则
- **不触碰**目标板的 Flash 或 RAM 内容
