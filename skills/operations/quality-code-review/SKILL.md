...
**证据**：
- F411 上 UART ORE 溢出在 ISR 中未清除标志位 → 系统无声卡死（bus-uart 有详细分析）
**应用**：每次代码审查先找 ISR
...
- `debug-diagnostic-framework` — 五层诊断模型
- `misra-c2012-standard` — 143 条 MISRA 规则
- `lixin-c-coding-standard-zh` — 立芯编码规范
- `quality-static-analysis` — cppcheck 静态分析工具
...
- `debug-diagnostic-framework` — 五层诊断方法论
- `misra-c2012-standard` — 全规则参考
- `lixin-c-coding-standard-zh` — 编码规范
- `quality-static-analysis` — cppcheck 工具集成
- `platform-cortex-registers` — HardFault 诊断流程
- `bsp-device-adaptation/references/oop-patterns-in-c.md` — 3W 设计决策模型、OOP 四模式、设计决策树、反模式清单
- 各外设模块 (bus-i2c/bus-spi/bus-uart/peripheral-timer/peripheral-adc/peripheral-dma)
