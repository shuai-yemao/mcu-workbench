...
### 不该激活
- 用户只需要烧录**单块**目标板 → 优先用 `tool-flash-jlink` / `tool-flash-openocd` / `tool-flash-esp-idf` 等单板烧录 skill
...
- 上游：`tool-build-keil` / `tool-build-cmake`（先编译出固件）
- 上游：`platform-option-bytes`（量产前配置 RDP/WRP 保护）
- 单板验证：先用 `tool-flash-jlink` / `tool-flash-keil` 验证单板烧录通过
