...
- **输出后端**：SEGGER RTT（需 `observability-rtt-porting`）或 UART printf
...
- **不覆盖文件系统写入** — 文件后端需要文件系统支持（`middleware-fatfs`）
- **不覆盖 Flash 存储日志** — Flash 后端需要 `middleware-sfud` 或 `platform-internal-flash`
- **不覆盖 CmBacktrace** — elog 是常规日志，故障转储由 `debug-crash-backtrace` 处理
- 与 `debug-crash-backtrace` 互补：elog 输出常规运行日志，CmBacktrace 输出故障诊断
- 与 `observability-rtt-porting` 配合：elog 通过 RTT 输出

## 交接

- **RTT 后端不工作** → 使用 `observability-rtt-porting` skill 单独测试 RTT
- **需要文件写入** → 使用 `middleware-fatfs` + `middleware-sfud` 对接文件后端插件
- **需要故障自动追踪** → 使用 `debug-crash-backtrace` skill
