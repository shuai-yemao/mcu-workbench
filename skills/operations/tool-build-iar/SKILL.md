---
name: tool-build-iar
description: 当需要通过 IAR Embedded Workbench 命令行编译嵌入式工程，调用自带脚本解析工程文件、执行构建并定位固件产物时使用。当用户提到 IAR 编译、EWARM、iccarm、IAR Embedded Workbench、IAR 构建时使用。
version: "1.0.0"
---
...
### 不该激活
- 工程中存在 `CMakeLists.txt` 但无 `.ewp`/`.eww` → 优先用 `tool-build-cmake`
- 工程中存在 `.uvprojx`/`.uvproj` → 优先用 `tool-build-keil`
- 工程中存在 `platformio.ini` → 优先用 `tool-build-platformio`
- 用户在非 Windows 平台上 → IAR 仅 Windows 可用，应使用 `tool-build-cmake` + ARM GCC

### 不该做
- **禁止**在非 Windows 平台尝试执行编译（工程解析和产物扫描仍可执行）
- **禁止**修改 .ewp/.eww 工程文件
- **禁止**在多个 configuration 候选时静默选择

### 不该碰
- **不触碰** IAR 安装目录
- **不触碰** .ewp/.eww 工程文件：只读取，不修改
