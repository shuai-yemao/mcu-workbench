...
同步脚本安装 **109 catalog / 31 canonical** Skills；当前 OS/BSP/Core/MCU 入口为 `platform_os`、`impl_os`、`platform_bsp`、`impl_board`、`platform_mcu`、`vendor_stm32`。旧名称只按兼容映射迁移到这些 canonical 目录；发生新旧目录并存或多个旧别名冲突时会停止覆盖，并在替换前创建备份。
