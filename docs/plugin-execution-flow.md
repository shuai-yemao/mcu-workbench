...
“外部 Flash 驱动怎么分层”
    ...
    → 执行中按需查阅 bsp-port / bsp-hal-driver / bsp-handler（器件链路）、platform_os / os-runtime（任务/队列/Runtime）、platform_mcu / vendor_stm32（底层外设）

当前 active 目录为 **109 catalog / 31 canonical**；`platform_os`、`os-runtime`、`platform_bsp`、`bsp-port`、`platform_mcu` 与 `vendor_stm32` 是当前入口，旧名仅作为兼容映射。
```
