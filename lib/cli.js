...
const mcuClaudeLayer = require('../commands/mcu-claude-layer');
const {
  getAllSkills,
  getSkillsByCategory,
  getSkillsByPlatform,
...
const COMMANDS = {
  ...
  'claude-layer': mcuClaudeLayer
};

const BOOLEAN_OPTIONS = new Set(['help', 'json', 'version', 'execute', 'clean', 'all', 'write', 'force', 'strict', 'allow-custom-device']);
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (!command) {
      command = token;
    } else if (command === 'claude-layer' && !options.action) {
...
function validateOptions(command, options) {
  ...
  const allowed = new Set([
    ...
    ...((command === 'build' || command === 'flash' || command === 'debug' || command === 'monitor') ? ['execute'] : []),
    ...(command === 'skills' ? ['all'] : []),
    ...(command === 'claude-layer' ? ['action'] : [])
  ]);
  if (command === 'driver') allowed.add('peripheral');
...
function getHelp(command) {
  if (!command) {
    return [
      ...
      '  core       Generate one MCU Core peripheral abstraction',
      ...
      '  claude-layer <action>  Manage Claude layer rules (init, scan, sync, validate)',
  ...
  if (canonical === 'claude-layer') {
    return [
      'Usage: mcu-workbench claude-layer <init|scan|sync|validate> --root <firmware-root> [options]',
      '',
      'Options:',
      '  --config <path>  Layer configuration relative to --root',
      '  --write          Apply init or sync changes after printing the plan',
      '  --strict         Treat unverified paths as validation errors',
  ...
  if (['build', 'flash', 'debug', 'monitor'].includes(canonical)) lines.push('  --execute        Run the generated external command');
  return lines.join('\n');
}
...
function printHuman(command, result, stdout) {
  ...
  if (command === 'claude-layer') {
    stdout(`Claude layer ${result.action}: ${result.changes ? result.changes.length : 0} planned changes.`);
    for (const change of result.changes || []) stdout(`  ${change.operation}: ${change.path}`);
    for (const finding of result.errors || []) stdout(`  error ${finding.code}: ${finding.file || ''} ${finding.message}`.trim());
    for (const finding of result.warnings || []) stdout(`  warning ${finding.code}: ${finding.file || ''} ${finding.message}`.trim());
    return;
  }
  if (command === 'driver' && result.manifest) {
    stdout(`Manifest: ${JSON.stringify(result.manifest)}`);
  }
  if (result.files) {
    const subject = result.peripheral || result.device || 'layered BSP';
    stdout(`Generated ${result.files.length} files for ${subject}`);
    if (command === 'driver') result.files.forEach((file) => stdout(`  ${file.path}`));
  }
  if (result.output) stdout(result.output);
  if (result.probe) stdout(`Probe: ${result.probe}`);
  if (result.openocdCommand) stdout(`OpenOCD: ${result.openocdCommand}`);
  if (result.jlinkServerCommand) stdout(`J-Link GDB Server: ${result.jlinkServerCommand}`);
  if (result.gdbCommand) stdout(`GDB: ${result.gdbCommand}`);
  if (result.channel) {
    stdout(`Monitor channel: ${result.channel}`);
    if (result.command) stdout(`Monitor command: ${result.command}`);
  } else {
...
async function runCli(argv = process.argv.slice(2), io = {}) {
  ...
  try {
    ...
    if (command === 'driver' && parsed.options.peripheral !== undefined) {
      mcuDriver.rejectLegacyPeripheral(parsed.options);
    }
    ...
    return { exitCode: result.exitCode || 0, result };
  } catch (error) {
    stderr(`Error: ${error.message}`);
    stderr('Use --help to see valid commands and options.');
    const exitCodes = { USAGE: 2, LAYER: 3, COLLISION: 4 };
    return { exitCode: exitCodes[error.code] || 1, error };
  }
}
