...
function commentLine(text, indent = '', marker = '/*') {
  const suffix = ' */';
  const prefix = `${indent}${marker} ${text} `;
  const padding = Math.max(1, 80 - prefix.length - suffix.length);
  return `${prefix}${'-'.repeat(padding)}${suffix}`;
}

function sectionComment(name) {
  return commentLine(name);
}
...
function addSectionComments(content) {
  ...
  return content.replace(sections, (match, indent, name) => `${indent}${sectionComment(name)}`);
}
...
function stepDescription(functionInfo) {
  const source = `${functionInfo.name} ${functionInfo.body}`;
  if (/goto\s+cleanup_|cleanup_/.test(source)) {
    return '按资源依赖顺序初始化，失败时执行逆序清理。';
  }
  if (/event_pending|pending_event|pending_status/.test(source)) {
    return '更新待处理事件状态，交由任务上下文继续处理。';
  }
  if (/callback/.test(source)) {
    return '保存或调用事件回调及其用户上下文。';
  }
  if (/pf_[A-Za-z_]\w*\s*\(/.test(source)) {
    return '校验依赖后调用已注入的底层操作。';
  }
  if (/==\s*NULL|!\s*[A-Za-z_]\w*\.is_inited|!\s*[A-Za-z_]\w*->/.test(source)) {
    return '检查输入参数、实例状态和必需的操作回调。';
  }
  if (/is_inited|s_[A-Za-z_]\w*\s*=/.test(source)) {
    return '初始化实例状态并绑定公开操作。';
  }
  return '完成本函数的核心状态更新并返回操作结果。';
}

function addStepComments(content) {
  for (const functionInfo of [...findGeneratedFunctions(content)].reverse()) {
    if (functionInfo.openBrace < 0) continue;
    const step = commentLine(stepDescription(functionInfo), '    ');
    const bodyStart = functionInfo.openBrace + 1;
    const body = content.slice(bodyStart);
    if (/^\s*\/\*[^*]/.test(body)) continue;
    content = `${content.slice(0, bodyStart)}\n${step}${content.slice(bodyStart)}`;
  }
  return content;
}
...
function applyGeneratedCommentContract(content) {
  let documented = addSectionComments(content);
  documented = addMemberComments(addTypeBriefs(addMacroComments(documented)));
...
function formatGeneratedCode(content, fileName) {
  ...
  for (const executable of candidates) {
    ...
    if (result.status === 0) {
      let formatted = alignTrailingComments(result.stdout.trimEnd());
      const guardMatch = formatted.match(/#define\s+([A-Z0-9_]+_H)\b/);
      if (guardMatch) formatted = formatted.replace(/#endif\s*$/, `#endif /* ${guardMatch[1]} */`);
      return `${formatted}\n`;
    }
  }
