...
function addMultilineFunctionSubsections(content) {
  ...
  for (const declaration of declarations.reverse()) {
    ...
    if (new RegExp(`\\/\\* ${group} -+ \\*\\/`).test(sectionContent)) continue;
      ...
      if (steps.length > 0) {
        ...
        for (const step of steps) {
          ...
          const isGuardFailureReturn = step === '结果'
            && currentStep === '校验'
            && indent.length > 4;
          if (!isGuardFailureReturn && !hasStepComment(previous, step)) {
...
function generateModelFirstBsp({ deviceType, device, cores }) {
  ...
  const driverRoot = `04_Impl/impl_bsp/${deviceType}/${device.directory}`;
  const handleRoot = `04_Impl/impl_bsp_handler/${deviceType}`;
  const portRoot = `04_Impl/impl_board/${deviceType}`;
  ...
  const files = [
    ...
    { path: `${portRoot}/Inc/impl_${deviceType}_port.h`, content: portHeaderV2(deviceType) },
    { path: `${portRoot}/Src/impl_${deviceType}_port.c`, content: portSourceV2(deviceType, device, primaryCore) }
  ];
