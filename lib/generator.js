...
function parameterDirection(parameter, name) {
  ...
  if (/^(?:callback|context|driver|handle|instance|ops|tx_data)$/i.test(name)) return '[in]';
  return '[in,out]';
}
...
function returnDescription(value) {
  if (/^0$|PLATFORM_ERR_OK/.test(value)) return '操作成功。';
  if (/^-1$|PLATFORM_ERR_PARAM/.test(value)) return '参数非法、实例未就绪或底层操作失败。';
...
function addStepComments(content) {
  for (const functionInfo of [...findGeneratedFunctions(content)].reverse()) {
    ...
    const step = `    /* ${stepDescription(functionInfo)} ---------------------------------------------- */`;
