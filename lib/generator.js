...
typedef struct {
    uint32_t event_id;
    platform_error_t status;
    uint32_t sequence;
} ${prefix}_event_t;

typedef struct {
    void *backend_context;
    platform_error_t (*pf_init)(void *backend_context);
    platform_error_t (*pf_transfer)(void *backend_context, const void *tx_data, void *rx_data, size_t length, uint32_t timeout_ms);
    platform_error_t (*pf_start_async)(void *backend_context, const void *tx_data, void *rx_data, size_t length);
    platform_error_t (*pf_cancel)(void *backend_context);
} ${prefix}_t;

/* Public Functions */
/** @brief Initializes an injected MCU ${core.toUpperCase()} backend. */
platform_error_t ${prefix}_init(${prefix}_t *instance);
/** @brief Executes a synchronous transaction with timeout expressed in milliseconds. */
platform_error_t ${prefix}_transfer(${prefix}_t *instance, const void *tx_data, void *rx_data, size_t length, uint32_t timeout_ms);
/** @brief Starts an optional asynchronous transaction. */
platform_error_t ${prefix}_start_async(${prefix}_t *instance, const void *tx_data, void *rx_data, size_t length);
/** @brief Cancels an optional asynchronous transaction. */
platform_error_t ${prefix}_cancel(${prefix}_t *instance);
/** @brief Converts an IRQ completion into a value event for upper layers. */
platform_error_t ${prefix}_irq_dispatch(${prefix}_t *instance, ${prefix}_event_t *event);
/** @brief Converts a DMA IRQ completion into a value event for upper layers. */
platform_error_t ${prefix}_dma_irq_dispatch(${prefix}_t *instance, ${prefix}_event_t *event);
...
typedef struct {
    void *backend_context;
    platform_error_t (*pf_init)(void *backend_context);
    platform_error_t (*pf_configure)(void *backend_context, uint32_t pin, uint32_t mode_flags);
    platform_error_t (*pf_set)(void *backend_context, uint32_t pin, bool level);
    platform_error_t (*pf_get)(void *backend_context, uint32_t pin, bool *level);
    platform_error_t (*pf_toggle)(void *backend_context, uint32_t pin);
} ${prefix}_t;
...
platform_error_t ${prefix}_configure(${prefix}_t *instance, uint32_t pin, uint32_t mode_flags);
/** @brief Sets one pin output level. */
platform_error_t ${prefix}_set_pin(${prefix}_t *instance, uint32_t pin, bool level);
/** @brief Reads one pin level. */
platform_error_t ${prefix}_get_pin(${prefix}_t *instance, uint32_t pin, bool *level);
/** @brief Toggles one pin output level. */
platform_error_t ${prefix}_toggle_pin(${prefix}_t *instance, uint32_t pin);
