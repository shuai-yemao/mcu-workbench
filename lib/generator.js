...
function commentLine(text, indent = '', marker = '/*', width = 80) {
  const suffix = ' */';
...
function sectionComment(name, width = 80) {
...
function macroGroup(name) {
  if (/_ERROR|_ERR|_OK|_SUCCESS/.test(name)) return '返回值';
    ...
    if (macroMatch && !/_H$/.test(macroMatch[1]) && isMacroSection) {
      ...
      if (group !== currentGroup) {
        const insertAt = result.length > 0 && /^\s*\/\*/.test(result[result.length - 1])
          ? result.length - 1
          : result.length;
        result.splice(insertAt, 0, sectionComment(group, 40));
        currentGroup = group;
      }
    }
...
function stepTitle(functionInfo) {
...
function addStepComments(content) {
  for (const functionInfo of [...findGeneratedFunctions(content)].reverse()) {
    ...
    const step = commentLine(stepTitle(functionInfo), '    ', '/*', 20);
...
function formatGeneratedCode(content, fileName) {
  ...
  for (const executable of candidates) {
    ...
    if (result.status === 0) {
      let formatted = alignGeneratedCode(result.stdout.trimEnd());
      formatted = alignTrailingComments(formatted);
  ...
  return `${alignTrailingComments(alignGeneratedCode(addMemberComments(documented.trimEnd())))}\n`;
}
