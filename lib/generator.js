...
function ssd1306Config() {
  return `${workflowFileHeader('impl_ssd1306_config.h', 'SSD1306 器件配置。', 'Port 将器件几何参数注入显示句柄。')}
...
function ssd1306DriverHeader() {
  return `${workflowFileHeader('impl_ssd1306_driver.h', 'SSD1306 协议驱动 API。', 'Port 注入 Core I2C 操作；驱动发出控制器命令与帧事务。', ['<stddef.h>', '<stdint.h>'])}
...
function ssd1306DriverSource() {
  return `${workflowFileHeader('impl_ssd1306_driver.c', 'SSD1306 协议实现。', '初始化发送 SSD1306 命令；帧刷新使用注入的事务级 Core I2C 操作。', ['impl_ssd1306_driver.h', 'impl_ssd1306_config.h'])}
/* Includes */
...
function displayHandleHeader() {
  return `${workflowFileHeader('impl_display_handle.h', '显示类句柄 API。', 'Port 注入 OSAL 与驱动操作；句柄持有帧缓冲并串行化显示请求。', ['<stdbool.h>', '<stddef.h>', '<stdint.h>'])}
...
function displayHandleSource() {
  return `${workflowFileHeader('impl_display_handle.c', '显示类帧缓冲与串行化实现。', '句柄持有帧缓冲，用注入的 OSAL 操作加锁，再调用通用驱动操作。', ['impl_display_handle.h', '<string.h>'])}
/* Includes */
...
function displayWrapperHeader() {
  return `${workflowFileHeader('platform_display_wrapper.h', '平台无关显示封装层。', 'APP 调用稳定的显示 API；封装层通过已注册的上下文优先操作表转发。', ['<stdbool.h>', '<stdint.h>'])}
...
function displayWrapperSource() {
  return `${workflowFileHeader('platform_display_wrapper.c', '显示封装层转发实现。', '封装层仅存储抽象公共操作表，绝不引用 Port、Core、Handler 或 Driver。', ['platform_display_wrapper.h'])}
/* Includes */
...
function displayPortHeader() {
  return `${workflowFileHeader('impl_display_port.h', '显示 BSP Port 注册 API。', 'Port 创建 OSAL 资源、注入 Core 与驱动操作，然后注册 Wrapper 公共操作。', ['<stdint.h>'])}
...
function displayPortSource() {
  return `${workflowFileHeader('impl_display_port.c', 'SSD1306 显示组合根。', 'Port 创建 OSAL 互斥锁，向句柄注入 Core 总线与驱动 API，并注册 Wrapper 操作。', ['osal.h', 'impl_ssd1306_driver.h', 'impl_display_handle.h', 'platform_display_wrapper.h'])}
/* Includes */
