...
const DEFAULT_LAYOUT = {
  ...
  bsp: [/(?:^|\/)Bsp(?:\/|$)/i],
  bspDriver: [/^Bsp\/(?:Driver|Drivers|Borad_drive|Board_drive)(?:\/|$)/i],
  bspPort: [/^Bsp\/(?:Port|Porting)(?:\/|$)/i, /^Bsp\/.*\/[^/]*port[^/]*\.[ch]$/i],
  wrapper: [/(?:^|\/)(?:Wrapper|Wapper)(?:\/|$)/i, /(?:^|\/)[^/]*(?:wrapper|wapper)[^/]*\.[ch]$/i],
  corePublic: [/^Core\/(?:.*\/)?(?:Inc|Include)\/.*\.h$/i, /^Core\/[^/]+\.h$/i],
  osWrapper: [/(?:^|\/)os_adapter\/(?:inc|shared)(?:\/|$)/i, /(?:^|\/)osal_[^/]*\.[ch]$/i],
  osPort: [/(?:^|\/)os_adapter\/(?:FreeRTOS|RT-Thread|RTThread|BareMetal)(?:\/|$)/i]
};
...
const SOFT_I2C_PRIMITIVE = /\b[A-Za-z0-9_]*(?:i2c|iic)[A-Za-z0-9_]*(?:start|stop|wait_?ack|send_?(?:not_?|no_?)?ack|send_?byte|receive_?byte)\w*\s*\(/gi;
const SOFT_I2C_DECLARATION = /^[ \t]*(?:(?:static|inline|extern)\s+)*(?:[A-Za-z_]\w*[ \t*]+)+(?:[A-Za-z0-9_]*(?:i2c|iic)[A-Za-z0-9_]*(?:start|stop|wait_?ack|send_?(?:not_?|no_?)?ack|send_?byte|receive_?byte)\w*)\s*\(/gim;
const CONCRETE_WRAPPER_TYPE = /#\s*include\s*[<"][^>"]*(?:driver|handler)[^>"]*[>"]|\b\w+_(?:driver|handler)_t\b/g;
const PERSISTENT_DEVICE_INSTANCE = /^[ \t]*static\s+(?:(?:const|volatile)\s+)*(?:struct\s+)?[A-Za-z_]\w*_(?:driver|handler)_t\s+\**[A-Za-z_]\w*\s*(?:\[[^\]]*\])?\s*[;=]/gim;
const PERSISTENT_DEVICE_STATE = /^[ \t]*static\s+(?:(?:const|volatile)\s+)*(?:struct\s+)?[A-Za-z_]\w*(?:\s*\*+)?\s+[A-Za-z_]\w*(?:latest|cache|temperature|humidity)[A-Za-z_0-9]*\s*(?:\[[^\]]*\])?\s*[;=]/gim;
...
function addPortVendorFindings(findings, { content, file }) {
  ...
  for (const match of content.matchAll(expression)) {
    ...
    const details = lineDetails(content, match.index);
    let ruleId = 'BSP_PORT_UNCLASSIFIED_VENDOR_CALL';
    let severity = 'error';
    let message = 'Port HAL calls must be limited to board configuration or moved to a Core backend.';
    if (HAL_CONFIGURATION_CALL.test(call)) {
      ruleId = 'BSP_PORT_VENDOR_CONFIG_CALL';
      severity = 'warning';
      message = 'Port HAL configuration is allowed but must remain board binding, not device protocol.';
    } else if (HAL_BUS_TRANSACTION_CALL.test(call)) {
      ruleId = 'BSP_PORT_DEVICE_PROTOCOL_CALL';
      message = 'Port must not execute bus transactions; move device protocol and transport execution to Driver/Core.';
    }
    if (!findings.some((finding) => finding.ruleId === ruleId
      && finding.file === file
...
function validateArchitectureContract({ root, layout } = {}) {
  ...
  for (const filePath of files) {
    ...
    if (isBspPort) {
      ...
      addMatches(findings, {
        content: code, file, ruleId: 'BSP_PORT_PERSISTENT_DEVICE_INSTANCE',
        message: 'Port must construct into Wrapper-owned storage rather than retain Driver, Handler, or device cache state.',
        pattern: PERSISTENT_DEVICE_INSTANCE
      });
      addMatches(findings, {
        content: code, file, ruleId: 'BSP_PORT_PERSISTENT_DEVICE_INSTANCE',
        message: 'Port must construct into Wrapper-owned storage rather than retain Driver, Handler, or device cache state.',
        pattern: PERSISTENT_DEVICE_STATE
      });
    }
