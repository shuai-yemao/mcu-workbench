...
function addOsWrapperForwardingFindings(findings, { content, file }) {
  const definition = /\b[A-Za-z_]\w*[\w\s*]*\b(osal_([A-Za-z0-9_]+))\s*\([^;{}]*\)\s*\{([\s\S]*?)\}/g;
  for (const match of content.matchAll(definition)) {
    const expected = `os_${match[2]}_impl`;
    if (new RegExp(`\\b${expected}\\s*\\(`).test(match[3])) continue;
    const details = lineDetails(content, match.index);
    findings.push({
      ...
      file,
      line: details.line,
      message: `OS Wrapper ${match[1]} must forward to ${expected}().`,
      evidence: details.evidence
    });
  }
}
