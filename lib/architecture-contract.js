...
const DEFAULT_LAYOUT = {
  ...
  app: [/(?:^|\/)(?:App|Application|User_Task)(?:\/|$)/i, /(?:^|\/)01_App(?:\/|$)/i],
  middleware: [/(?:^|\/)Middlewares?(?:\/|$)/i, /(?:^|\/)02_Service(?:\/|$)/i],
  bsp: [/(?:^|\/)Bsp(?:\/|$)/i, /(?:^|\/)03_Platform\/platform_bsp(?:\/|$)/i, /(?:^|\/)04_Impl\/impl_(?:bsp|bsp_handler|board)(?:\/|$)/i],
  bspDriver: [/^Bsp\/(?:Driver|Drivers|Borad_drive|Board_drive|BoardDriver)(?:\/|$)/i, /(?:^|\/)04_Impl\/impl_bsp(?:\/|$)/i],
  bspHandler: [
    /^Bsp\/(?:Handler|Handle|Handlers)(?:\/|$)/i,
    /^Bsp\/BoardDriver\/[^/]+\/(?:Handler|Handle|Handlers)(?:\/|$)/i,
    /(?:^|\/)04_Impl\/impl_bsp_handler(?:\/|$)/i,
    /(?:^|\/)04_Impl\/impl_bsp\/impl_bsp_handle(?:\/|$)/i
  ],
  bspPort: [
    /^Bsp\/(?:Port|Porting)(?:\/|$)/i,
    /^Bsp\/.*\/[^/]*port[^/]*\.[ch]$/i,
    /(?:^|\/)04_Impl\/impl_board(?:\/|$)/i,
    /(?:^|\/)04_Impl\/impl_bsp\/impl_bsp_port(?:\/|$)/i
  ],
  bspWrapper: [/^Bsp\/Wrapper(?:\/|$)/i, /^Bsp\/.*\/[^/]*wrapper[^/]*\.[ch]$/i, /(?:^|\/)03_Platform\/platform_bsp\/.*wrapper[^/]*\.[ch]$/i],
  wrapper: [/(?:^|\/)Wrapper(?:\/|$)/i, /(?:^|\/)[^/]*wrapper[^/]*\.[ch]$/i],
  appFacade: [/(?:^|\/)User_Task\/.*\/Platform\/[^/]*_port(?:\/|$)/i],
  corePublic: [/^Core\/(?:.*\/)?(?:Inc|Include|Public)\/.*\.h$/i, /^Core\/[^/]+\.h$/i, /(?:^|\/)03_Platform\/platform_mcu(?:\/.*)?\/(?:Inc|Include|Public)\/.*\.h$/i, /(?:^|\/)03_Platform\/platform_mcu\/[^/]+\.h$/i],
  osWrapper: [/(?:^|\/)os_adapter\/(?:inc|shared)(?:\/|$)/i, /^OS\/Wrapper(?:\/|$)/i, /(?:^|\/)osal_[^/]*\.[ch]$/i, /(?:^|\/)04_Impl\/impl_os\/(?:inc|shared|wrapper)?(?:\/|$)/i],
  osPort: [/(?:^|\/)os_adapter\/(?:FreeRTOS|RT-Thread|RTThread|BareMetal)(?:\/|$)/i, /^OS\/Port(?:\/|$)/i, /(?:^|\/)04_Impl\/impl_os\/(?:port|ports?|freertos|rtthread|baremetal)(?:\/|$)/i, /(?:^|\/)04_Impl\/impl_os\/(?:inc|shared)\/impl_os_(?:freertos|rtthread|baremetal)\.[ch]$/i]
};
...
const BSP_HANDLER_INJECTION_DEPENDENCY = /#\s*include\s*[<"][^>"]*(?:drv_adapter_(?:port|wrapper)|core_|mcu_|platform_(?:mcu|os|adc|dma|event|gpio|i2c|irq|reset_reason|spi|tick|timer|uart|watchdog)|stm32|hal|freertos|rtthread|osal_internal|(?:bsp|impl)_[^>"]*_driver)[^>"]*[>"]/gi;
...
function findFunctionDefinitions(content) {
  ...
  const definitions = [];
  const signature = /(?:^|\n)[ \t]*(?:(?:static|inline|extern)\s+)*(?:[A-Za-z_]\w*[ \t*]+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*\{/g;
...
function maskKnownUnreachableBranches(content) {
  const masked = content.split('');
  const expression = /\bif\s*\(\s*(?:0(?:[uUlL]+)?|false)\s*\)\s*\{/g;
  for (const match of content.matchAll(expression)) {
...
function collectReachableFunctionCode(entry, definitions) {
  ...
  const visit = (definition) => {
    ...
    const body = maskKnownUnreachableBranches(definition.body);
    reachable.push(body);
    for (const match of body.matchAll(/\b([A-Za-z_]\w*)\s*\(/g)) {
      const callee = byName.get(match[1]);
...
function addOsWrapperForwardingFindings(findings, { content, file }) {
  for (const definition of findFunctionDefinitions(content)) {
    const match = definition.name.match(/^osal_([A-Za-z0-9_]+)$/);
    if (!match) continue;
    const expected = `os_${match[1]}_impl`;
    if (new RegExp(`\\b${expected}\\s*\\(`).test(definition.body)) continue;
    const details = lineDetails(content, definition.index);
    findings.push({
      ruleId: 'OS_WRAPPER_IMPL_FORWARDING',
      severity: 'error',
      file,
      line: details.line,
      message: `OS Wrapper ${definition.name} must forward to ${expected}().`,
      evidence: details.evidence
    });
  }
}
...
function validateArchitectureContract({ root, layout } = {}) {
  ...
  for (const filePath of files) {
    ...
    const isBspDriver = matchesRole(file, roles.bspDriver) && !isBspHandler && !isBspPort;
    ...
    if (isAppFacade) {
      addMatches(findings, {
        ...
        message: 'APP Facades may forward only to Platform public APIs, not concrete BSP/Core/RTOS dependencies.',
    ...
    if (isBsp && !isBspPort) {
      addMatches(findings, {
        ...
        message: 'BSP Driver and Handle code must use injected OS/Platform Ops instead of native RTOS APIs.',
        pattern: NATIVE_RTOS
      });
      addMatches(findings, {
        content, file, ruleId: 'BSP_VENDOR_CALL',
        message: 'BSP Driver and Handle code must use injected Platform capabilities instead of vendor HAL calls.',
        pattern: VENDOR_CALL
      });
      addMatches(findings, {
    ...
    if (isBspPort) {
      ...
      const definitions = findFunctionDefinitions(code);
      const registrations = definitions.filter((definition) => /_port_register$/.test(definition.name));
      for (const registration of registrations) {
        const registrationCode = collectReachableFunctionCode(registration, definitions);
        const modelFirst = /\b[A-Za-z_]\w*_resource_get_ops\s*\(|\b[A-Za-z_]\w*_handle_construct\s*\(|\bplatform_[A-Za-z0-9_]+_register_default\s*\(/.test(registrationCode);
        const requiredInjections = modelFirst ? [
          ['BSP_PORT_CORE_OPS_INJECTION', /\b[A-Za-z_]\w*_resource_get_ops\s*\(/],
          ['BSP_PORT_MCU_OPS_INJECTION', /\b[A-Za-z_]\w*_resource_get_ops\s*\(/],
          ['BSP_PORT_OS_WRAPPER_OPS_INJECTION', /\b[A-Za-z_]\w*_handle_construct\s*\(/],
          ['BSP_PORT_HAL_DRIVER_OPS_INJECTION', /\b[A-Za-z_]\w*_driver_construct\s*\(/],
          ['BSP_PORT_MODEL_OPS_BINDING', /\.[A-Za-z0-9_]+\s*=\s*impl_[A-Za-z0-9_]+_handle_[A-Za-z0-9_]+\s*,/],
          ['BSP_PORT_MODEL_REGISTRATION', /\bplatform_[A-Za-z0-9_]+_register_default\s*\(/]
        ] : [
        ...
        for (const [ruleId, pattern] of requiredInjections) {
          if (!pattern.test(registrationCode)) {
            findings.push({
              ruleId,
              severity: 'error',
              file,
              line: lineDetails(code, registration.index).line,
              message: 'BSP Port must compose and inject the required operation table.',
              evidence: registration.name
            });
          }
        }
      }
    ...
    if (isBspHandler) {
      addMatches(findings, {
        ...
        message: 'BSP Handle must use injected OS and Driver Ops rather than concrete implementations.',
    ...
    if (isBspDriver) {
      addMatches(findings, {
        ...
        message: 'BSP HAL Driver may use Platform public contracts but must not include HAL, RTOS, or OS Wrapper dependencies.',
    ...
    if (isOsWrapper) {
      addOsWrapperForwardingFindings(findings, { content: code, file });
      addMatches(findings, {
