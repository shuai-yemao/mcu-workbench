...
function validateArchitectureContract({ root, layout } = {}) {
  ...
  for (const filePath of files) {
    ...
    if (isBspPort) {
      addMatches(findings, {
        content, file, ruleId: 'BSP_PORT_VENDOR_CALL',
        message: 'BSP Port must compose Core capabilities instead of calling vendor HAL directly.',
        pattern: VENDOR_CALL
      });
      addMatches(findings, {
        content, file, ruleId: 'BSP_PORT_SOFT_I2C_PRIMITIVE',
