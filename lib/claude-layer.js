...
const { PLUGIN_OUTPUT_ROOT } = require('./project-output-paths');
...
const REPORT_RELATIVE_PATH = `${PLUGIN_OUTPUT_ROOT}/architecture/claude-layer-map.md`;
...
function planArtifacts(root, artifacts) {
  ...
  // 一次性迁移旧版分散目录：仅删除本插件旧路径下的受管规则，
  // 不删除 .claude/rules 下的其他用户规则，也不删除旧 docs/architecture 报告。
