...
function createDefaultConfig(root) {
  return {
    ...
    readme: {
      enabled: true,
      roots: [...DEFAULT_README_ROOTS],
      maxDepth: 2,
      excludeDirectories: [...DEFAULT_README_EXCLUDED_DIRECTORIES]
    },
    managed: {
...
function normalizeReadmeConfig(config, root) {
  const defaults = createDefaultConfig(root).readme;
  const original = config.readme || {};
  const normalized = {
    enabled: original.enabled !== false,
    roots: Array.isArray(original.roots) ? original.roots : defaults.roots,
    maxDepth: Number.isInteger(original.maxDepth) && original.maxDepth >= 0 ? original.maxDepth : defaults.maxDepth,
    excludeDirectories: [...new Set([
      ...defaults.excludeDirectories,
      ...(Array.isArray(original.excludeDirectories) ? original.excludeDirectories : [])
    ])]
  };
  const migrated = !config.readme
    || normalized.enabled !== original.enabled
    || JSON.stringify(normalized.roots) !== JSON.stringify(original.roots)
    || normalized.maxDepth !== original.maxDepth
    || JSON.stringify(normalized.excludeDirectories) !== JSON.stringify(original.excludeDirectories);
  return {
    config: { ...config, readme: normalized },
    migrated
  };
}
...
function scanProject({ root, config = createDefaultConfig() } = {}) {
  ...
  const normalizedReadme = normalizeReadmeConfig(config, resolvedRoot).config;
  ...
  const readmeScope = readmeDirectoryEntries(resolvedRoot, normalizedReadme);
  const readme = {
    enabled: normalizedReadme.readme.enabled !== false,
    roots: normalizedReadme.readme.roots || [],
    maxDepth: normalizedReadme.readme.maxDepth,
    excludeDirectories: normalizedReadme.readme.excludeDirectories || [],
    entries: readmeScope.entries,
    missingRoots: readmeScope.missingRoots
  };
  const digestInput = {
    project,
    ...
    unverified,
    readme: {
      enabled: readme.enabled,
      roots: readme.roots,
      maxDepth: readme.maxDepth,
      excludeDirectories: readme.excludeDirectories,
      entries: readme.entries.map((entry) => ({
        path: entry.path,
        layer: entry.layer,
        ...
        childDirectoryDetails: entry.childDirectoryDetails.map((child) => ({
          ...
          childDirectoryCount: child.childDirectoryCount,
          role: child.role,
      ...
      missingRoots: readme.missingRoots
    }
  };
  return {
    schemaVersion: 1,
...
function removeReadmeManagedBlock(content) {
  const expression = new RegExp(`${README_MANAGED_START.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[\\s\\S]*?${README_MANAGED_END.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
  return content.replace(expression, '').trimEnd();
}

function readmeManagedBlock(content) {
  const start = content.indexOf(README_MANAGED_START);
  const end = content.indexOf(README_MANAGED_END);
  if (start === -1 || end === -1 || end < start) return null;
  return content.slice(start, end + README_MANAGED_END.length);
}
...
function renderReadme(scan, entry, existingReadme) {
...
function renderReport(scan) {
  ...
  const unverified = scan.unverified.length
    ...
    : '- 无';
  const readmeRows = scan.readme.entries.map((entry) => `| \`${entry.path || '.'}\` | ${entry.readme ? '已有' : '缺失'} | ${entry.layer} | ${entry.childDirectories.length} |`).join('\n');
  const readmeMissingRoots = scan.readme.missingRoots.length ? scan.readme.missingRoots.map((item) => `\`${item}\``).join('、') : '无';
  return `# Claude 分层扫描报告\n\n## 工程证据\n\n- 构建系统：${scan.project.buildSystems.join('、') || '未确认'}\n- 配置证据：${scan.project.platformEvidence.map((item) => `\`${item.file}\` (${item.kind})`).join('、') || '未确认'}\n- 架构快照：\`${scan.architectureDigest}\`\n\n## 分层映射\n\n| 层 | 已确认目录 | 源文件数 | include 证据数 |\n|---|---|---:|---:|\n${rows.join('\n')}\n\n## README 管理范围\n\n- 管理深度：架构根目录及其下 ${scan.readme.maxDepth} 级目录\n- 排除目录：${scan.readme.excludeDirectories.map((item) => `\`${item}\``).join('、') || '无'}\n- 缺失配置根目录：${readmeMissingRoots}\n\n| 目录 | README | 架构层 | 直接子目录数 |\n|---|---|---|---:|\n${readmeRows || '| 无 | - | - | 0 |'}\n\n## 未确认路径\n\n${unverified}\n\n> 本报告来自静态扫描；不代表目标构建、烧录或实机验证已完成。\n`;
}
...
function artifactHash(relative, content) {
  ...
  if (relative === 'README.md' || relative.endsWith('/README.md')) return sha256(readmeManagedBlock(content) || '');
...
function buildRuleDesign(config, scan) {
  return {
    ...
    readme: {
      enabled: config.readme.enabled !== false,
      roots: config.readme.roots,
      maxDepth: config.readme.maxDepth,
      excludeDirectories: config.readme.excludeDirectories
    },
...
function buildArtifacts(root, config, scan, existingRoot, configWriteRelative, { bootstrap = false, qualityStyleWrite = false } = {}) {
  const artifacts = {};
  ...
  for (const entry of scan.readme.entries) {
    const relative = readmeRelativePath(entry.path);
    artifacts[relative] = renderReadme(scan, entry, readText(path.join(root, relative)));
  }
  ...
  const state = {
    ...
    unverified: scan.unverified,
    readme: {
      directories: scan.readme.entries.map((entry) => entry.path),
      missingRoots: scan.readme.missingRoots
    }
  };
...
function runClaudeLayer({ action, root, write = false, strict = false, configPath, rulesConfirmed = false } = {}) {
  const resolvedRoot = path.resolve(root || process.cwd());
  ...
  const normalizedReadme = normalizeReadmeConfig(rawConfig, resolvedRoot);
  const normalizedManaged = normalizeManagedConfig(normalizedReadme.config);
  const config = { ...normalizedManaged.config, architecture: architectureConfig.architecture };
  ...
  if (normalizedReadme.migrated && loadedConfig) {
    migrationWarnings.push({
      code: 'README_CONFIG_DEFAULTED',
      message: 'README 管理配置已补齐默认范围；请在 .mcu-workbench/claude-layer.json 中复核 roots、maxDepth 和 excludeDirectories。'
    });
  }
  ...
  const configWriteRelative = ((action === 'init' && !loadedConfig) || (action === 'bootstrap' && (!loadedConfig || rulesConfirmed)))
    ? CONFIG_RELATIVE_PATH
    : ((action === 'sync' && (layoutMigrated || normalizedReadme.migrated || normalizedManaged.migrated) && configSourceRelative) ? configSourceRelative : null);
...
module.exports = {
  ...
  README_MANAGED_END,
  DEFAULT_README_ROOTS,
  DEFAULT_README_EXCLUDED_DIRECTORIES,
  CLAUDE_RULE_PROFILE,
  DEFAULT_LAYER_ROOTS,
