...
const { pluginDashboardPath } = require('./project-output-paths');
...
function startDashboard(options = {}) {
  ...
  const outputPath = options.outputPath || pluginDashboardPath(root);
...
function statusDashboard(options = {}) {
  ...
  return { running: Boolean(current.pid), pid: current.pid, stale: current.stale, root, outputPath: pluginDashboardPath(root), paths: current.paths };
}
